...
class OperatorBase {
 public:
  ...
  virtual void Run(SupportWorkspace *ws) {
    NDLL_FAIL(name() + " is not a support operator!");
  }
 ...
 * RunImpl method and register the operator using the NDLL_REGISTER_OPERATOR
 * macro. To define meta-data about the op like the number of inputs
 * it takes, a docstring (for python), etc., use the NDLL_OPERATOR_SCHEMA,
...
NDLL_DECLARE_OPTYPE_REGISTRY(SupportOperator, OperatorBase);
