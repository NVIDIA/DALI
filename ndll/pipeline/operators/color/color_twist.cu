...
void ColorTwistBase<GPUBackend>::RunImpl(DeviceWorkspace *ws, const int idx) {
  ...
  NDLL_ENFORCE(IsType<uint8_t>(input.type()),
  ...
  for (int i = 0; i < input.ntensor(); ++i) {
    if (!augments_.empty()) {
      ...
      NDLL_CHECK_NPP(twist_func(input.tensor<uint8_t>(i),
                                nStep,
...
NDLL_REGISTER_OPERATOR(Brightness, BrightnessAdjust<GPUBackend>, GPU);
NDLL_REGISTER_OPERATOR(Contrast, ContrastAdjust<GPUBackend>, GPU);
NDLL_REGISTER_OPERATOR(Hue, HueAdjust<GPUBackend>, GPU);
NDLL_REGISTER_OPERATOR(Saturation, SaturationAdjust<GPUBackend>, GPU);
