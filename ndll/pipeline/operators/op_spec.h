...
#ifndef NDLL_PIPELINE_OPERATORS_OP_SPEC_H_
#define NDLL_PIPELINE_OPERATORS_OP_SPEC_H_
...
class OpSpec {
 public:
  ...
  inline std::string ArgumentInputName(int idx) const {
    ...
    NDLL_ENFORCE(idx_ptr != argument_inputs_indexes_.end(),
    ...
    NDLL_FAIL("Internal error - found argument input index for non-existent argument input.");
  }
  ...
  inline std::vector<T> GetRepeatedArgument(
    ...
    NDLL_ENFORCE(idx == 0, "Tensor arguments cannot be used for vector values");
...
#endif  // NDLL_PIPELINE_OPERATORS_OP_SPEC_H_
