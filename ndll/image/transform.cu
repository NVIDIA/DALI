...
__global__ void BatchedNormalizePermuteKernel(const uint8 *image_batch,
...
NDLLError_t BatchedNormalizePermute(const uint8 *image_batch,
  ...
  NDLL_ASSERT(image_batch != nullptr);
...
template NDLLError_t BatchedNormalizePermute<float16>(const uint8 *image_batch,
    int N, int H, int W, int C, float *mean, float *std, float16 *out_batch,
    cudaStream_t stream);

template NDLLError_t BatchedNormalizePermute<float>(const uint8 *image_batch,
