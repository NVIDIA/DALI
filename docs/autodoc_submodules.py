...
mod_aditional_doc = {
    ...
    "nvidia.dali.fn.readers": (
        ...
        " different source, and where the data locations are known at pipeline construction time via"
        " arguments. For data readers that are able to read from sources specified dynamically via"
        " regular inputs, see `nvidia.dali.fn.io` module."
    ),
    "nvidia.dali.fn.io": (
        "Operators in this module are data-reading operators that read data from a source specified "
        " at runtime by operator inputs. For inputless data readers that are able to build the dataset "
        " at pipeline constructions, see `nvidia.dali.fn.readers` module."
    )
}
