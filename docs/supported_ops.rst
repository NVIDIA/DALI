...
- **GPU** operator means that the operator can be scheduled on the GPU. Their outputs can only be used as
...
- **Sequences** means that the operator can produce or accept as an input a sequence, or video type of input.
- **Volumetric** means that the operator supports 3D data processing.

Reading this guide
^^^^^^^^^^^^^^^^^^^^

DALI operators are used to create the parametrized operator instance by using
its constructor and later to invoke its `__call__` operator
in the pipeline’s :meth:`~nvidia.dali.pipeline.Pipeline.define_graph` method.

In the documentation of for every DALI Operator, see the lists of **Keyword Arguments**
that are supported by the class constructor.

The documentation for ``__call__`` operator lists the positional arguments, (or parameters) and additional
keyword arguments. ``__call__`` should only be used in the :meth:`~nvidia.dali.pipeline.Pipeline.define_graph`.
The inputs to the ``__call__`` operator represent batches of Tensors (TensorLists) that are processed by DALI and
are returned by other DALI operators.
...
.. note::
    The names of the positional arguments for ``__call__`` operator (**parameters**) are provided only for
...
The following table lists all available operators and devices on which they can operate:
...
.. function:: Binary arithmetic operations: +, -, *, /, //
    ...
    .. note::
        The promotions type is commutative.

    .. note::
        The only allowed arithmetic operation between two ``bool`` values is multiplication
        ``(*)``.

    :rtype: TensorList of the type that is calculated based on the promotion rules type.
...
.. function:: Bitwise binary operations: &, |, ^
    ...
    The bitwise binary operations follow the same promotion rules type as arithmetic binary
    operations, but their inputs are restricted to integral types, such as ``bool``.
