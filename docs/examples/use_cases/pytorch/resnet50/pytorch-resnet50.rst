...
- **ndd**:
  Uses torchdata.nodes loader with DALI dynamic mode. See :ref:`ndd_torchdata` for a simple example.
...
.. code-block:: bash
   ...
   main.py [-h] [--arch ARCH] [-j N] [--epochs N] [--start-epoch N] [-b N] [--lr LR] [--momentum M] [--weight-decay W] [--print-freq N] [--resume PATH]
                  [-e] [--pretrained] [--dali_cpu] [--data_loader {pytorch,dali,dali_proxy,ndd}] [--prof PROF] [--deterministic] [--fp16-mode]
   ...
   options:
     ...
     --data_loader {pytorch,dali,dali_proxy,ndd}
                           Select data loader: "pytorch" for native PyTorch data loader,
                           "dali" for DALI data loader, "dali_proxy" for PyTorch dataloader with DALI proxy preprocessing,
                           or "ndd" for torchdata.nodes loader with dynamic mode.
