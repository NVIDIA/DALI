...
def input(tfrecord_files, tfrecord_idxs, device, shard_id, num_shards, random_shuffle=True):
    ...
    images = dali.fn.decoders.image(inputs["image/encoded"], device='cpu', 
    ...
    bboxes = dali.fn.transpose(dali.fn.stack(xmin, ymin, xmax, ymax), perm=[1, 0], device='cpu')
    classes = dali.fn.cast(inputs["image/object/class/label"], dtype=dali.types.INT32, device='cpu')
...
def random_crop_resize_2(device, images, bboxes, classes, output_size, scaling=[0.1, 2.0]):
    ...
    anchors, shapes, bboxes, classes = dali.fn.random_bbox_crop(
        ...
        input_shape = dali.fn.cast(dali.fn.cat(scaled_width, scaled_height, device='cpu'), dtype=dali.types.INT32, device='cpu'),
        bbox_layout = "xyXY",
        allow_no_crop = False
    )
