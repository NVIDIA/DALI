...
def learning_rate_schedule(params):
  ...
  if lr_decay_method == 'cosine':
    return CosineLrSchedule(params['adjusted_learning_rate'],
                            params['adjusted_lr_warmup_init'], params['lr_warmup_step'],
                            params['total_steps'])
...
class BoxLoss(tf.keras.losses.Loss):
  ...
  def call(self, y_true, box_outputs):
    ...
    box_loss = tf.cast(self.huber(box_targets, box_outputs, sample_weight=mask),
                       box_outputs.dtype)
    box_loss /= normalizer
    return box_loss
...
def detection_loss(cls_outputs, box_outputs, labels, config):
  ...
  if positives_momentum > 0:
    ...
    num_positives_sum = tf1.keras.backend.moving_average_update(
        ...
        num_positives_sum,
        momentum=config.positives_momentum)
  ...
  for level in levels:
    ...
    if config.data_format == 'channels_first':
      ...
      cls_targets_at_level = tf1.reshape(cls_targets_at_level,
                                        [bs, -1, width, height])
    else:
    ...
    if config.data_format == 'channels_first':
      cls_loss = tf1.reshape(cls_loss,
                            [bs, -1, width, height, config.num_classes])
    else:
      cls_loss = tf1.reshape(cls_loss,
                            [bs, width, height, -1, config.num_classes])
  ...
  box_loss = tf1.add_n(box_losses) if box_losses else tf1.constant(0.)

  total_loss = (
      cls_loss + config.box_loss_weight * box_loss)
