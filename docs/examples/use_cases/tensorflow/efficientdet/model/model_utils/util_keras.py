...
def average_name(ema, var):
    ...
    if var.ref() in ema._averages:  # pylint: disable=protected-access
        return ema._averages[var.ref()].name.split(":")[
            0
...
def restore_ckpt(model, ckpt_path_or_file, ema_decay=0.9998, skip_mismatch=True):
    ...
    if (
        tf.train.list_variables(ckpt_path_or_file)[0][0]
        ...
        if ema_decay > 0:
            ...
            var_dict = {average_name(ema, var): var for (ref, var) in ema_vars.items()}
        else:
            ema_vars = get_ema_vars(model)
...
def fp16_to_fp32_nested(input_nested):
    ...
    if isinstance(input_nested, tf.Tensor):
        ...
        out_tensor_dict = {k: fp16_to_fp32_nested(v) for k, v in input_nested.items()}
    else:
        return input_nested
