...
def update_learning_rate_schedule_parameters(params):
    ...
    batch_size = (
        ...
        if params["strategy"] in ["tpu", "gpus"]
...
def stepwise_lr_schedule(
    ...
    linear_warmup = adjusted_lr_warmup_init + (
        tf.cast(global_step, dtype=tf.float32)
        / lr_warmup_step
        * (adjusted_learning_rate - adjusted_lr_warmup_init)
    )
    learning_rate = tf.where(
        global_step < lr_warmup_step, linear_warmup, adjusted_learning_rate
    )
    lr_schedule = [
        [1.0, lr_warmup_step],
...
def cosine_lr_schedule(
    adjusted_lr, adjusted_lr_warmup_init, lr_warmup_step, total_steps, step
):
...
def polynomial_lr_schedule(
    adjusted_lr, adjusted_lr_warmup_init, lr_warmup_step, power, total_steps, step
):
...
def detection_loss(cls_outputs, box_outputs, labels, params):
    ...
    box_loss = tf.add_n(box_losses) if box_losses else tf.constant(0.0)
...
def reg_l2_loss(weight_decay, regex=r".*(kernel|weight):0$"):
    ...
    return weight_decay * tf.add_n(
        [tf.nn.l2_loss(v) for v in tf.trainable_variables() if var_match.match(v.name)]
    )
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        if params.get("clip_gradients_norm", None):
            ...
            with tf.name_scope("clip"):
                ...
                utils.scalar(
                    "gradient_norm", tf.linalg.global_norm(clipped_grads), is_tpu
                )
            ...
            if params["nms_configs"].get("pyfunc", True):
                ...
                for index in range(kwargs["boxes"].shape[0]):
                    detections = tf.numpy_function(
                        functools.partial(
                            nms_np.per_class_nms, nms_configs=nms_configs
                        ),
                        [
                            kwargs["boxes"][index],
                            kwargs["scores"][index],
