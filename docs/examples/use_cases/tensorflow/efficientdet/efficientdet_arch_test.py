...
class EfficientDetArchTest(tf.test.TestCase):
    def build_model(
        self, model_name, isize=None, is_training=False, data_format="channels_last"
    ):
    ...
    def test_efficientdet_d0(self):
        self.assertSequenceEqual(
            (3880067, 2535456183), self.build_model("efficientdet-d0", 512)
        )
    ...
    def test_efficientdet_d0_511_513(self):
        self.assertSequenceEqual(
            (3880067, 2613160475), self.build_model("efficientdet-d0", (511, 513))
        )

    def test_efficientdet_d1(self):
        self.assertSequenceEqual(
            (6625898, 6101276568), self.build_model("efficientdet-d1", 640)
        )

    def test_efficientdet_d1_640_1280(self):
        self.assertSequenceEqual(
            (6625898, 12194901743), self.build_model("efficientdet-d1", (640, 1280))
        )

    def test_efficientdet_d2(self):
        self.assertSequenceEqual(
            (8097039, 10993616292), self.build_model("efficientdet-d2", 768)
        )

    def test_efficientdet_d3(self):
        self.assertSequenceEqual(
            (12032296, 24882174639), self.build_model("efficientdet-d3", 896)
        )

    def test_efficientdet_d4(self):
        self.assertSequenceEqual(
            (20723675, 55167980877), self.build_model("efficientdet-d4", 1024)
        )

    def test_efficientdet_d5(self):
        self.assertSequenceEqual(
            (33653315, 135353202989), self.build_model("efficientdet-d5", 1280)
        )

    def test_efficientdet_d6(self):
        self.assertSequenceEqual(
            (51871782, 225532115747), self.build_model("efficientdet-d6", 1280)
        )

    def test_efficientdet_d7(self):
        self.assertSequenceEqual(
            (51871782, 324740293607), self.build_model("efficientdet-d7", 1536)
        )

    def test_efficientdet_lite0(self):
        self.assertSequenceEqual(
            (3243470, 979428213), self.build_model("efficientdet-lite0")
        )

    def test_efficientdet_lite1(self):
        self.assertSequenceEqual(
            (4248318, 1976353506), self.build_model("efficientdet-lite1")
        )

    def test_efficientdet_lite2(self):
        self.assertSequenceEqual(
            (5252334, 3386596870), self.build_model("efficientdet-lite2")
        )

    def test_efficientdet_lite3(self):
        self.assertSequenceEqual(
            (8350862, 7509226979), self.build_model("efficientdet-lite3")
        )

    def test_efficientdet_lite4(self):
        self.assertSequenceEqual(
            (15130894, 12953966715), self.build_model("efficientdet-lite4")
        )
...
class EfficientDetArchPrecisionTest(tf.test.TestCase):
    def build_model(self, features, is_training, precision):
        ...
        return utils.build_model_with_precision(precision, _model_fn, features, False)
...
class FreezeTest(tf.test.TestCase):
    def test_freeze(self):
        var_list = [
            tf.Variable(0.0, name="efficientnet"),
            tf.Variable(0.0, name="fpn_cells"),
            tf.Variable(0.0, name="class_net"),
        ]
        ...
        freeze_var_list = efficientdet_arch.freeze_vars(
            var_list, "(efficientnet|fpn_cells)"
        )
