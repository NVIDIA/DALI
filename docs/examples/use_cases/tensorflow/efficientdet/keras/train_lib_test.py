...
class TrainLibTest(tf.test.TestCase):
    def test_display_callback(self):
        ...
        display_callback = train_lib.DisplayCallback(
            sample_image, config.model_dir, update_freq=1
        )
    ...
    def test_lr_schedule(self):
        ...
        for i in range(5):
            ...
            self.assertEqual(
                legacy_fn.polynomial_lr_schedule(1e-3, 1e-4, 1, 2, 5, i), polynomial(i)
            )
    ...
    def test_losses(self):
        ...
        self.assertAllEqual(
            legacy_fn.focal_loss(box_outputs, box_targets, alpha, gamma, num_positives),
    ...
    def test_train_on_batch(self):
        ...
        self.assertAllClose(outputs, expect_results, rtol=0.1, atol=100.0)
    ...
    def test_fit(self):
        ...
        self.assertAllClose(hist.history["loss"], [26279.47], rtol=0.1, atol=10.0)
        self.assertAllClose(hist.history["det_loss"], [26279.47], rtol=0.1, atol=10.0)
        self.assertAllClose(hist.history["cls_loss"], [5065.437], rtol=0.1, atol=10.0)
        self.assertAllClose(hist.history["box_loss"], [424.0], rtol=0.1, atol=100.0)
        self.assertAllClose(hist.history["seg_loss"], [1.221547], rtol=0.1, atol=100.0)
    ...
    def test_recompute_grad(self):
        ...
        with tf.GradientTape() as tape:
            ...
            det_loss = model._detection_loss(
        ...
        with tf.GradientTape() as tape:
            ...
            det_loss2 = model._detection_loss(
                cls_outputs2, box_outputs2, labels, loss_vals
            )
