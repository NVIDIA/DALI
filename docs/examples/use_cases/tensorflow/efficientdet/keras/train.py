...
flags.DEFINE_integer("eval_samples", 5000, "The number of samples for " "evaluation.")
flags.DEFINE_integer(
    "steps_per_execution", 1000, "Number of steps per training execution."
)
flags.DEFINE_string(
...
flags.DEFINE_string(
    "val_file_pattern", None, "Glob for evaluation tfrecords (e.g., COCO val2017 set)"
)
flags.DEFINE_string(
    "val_json_file",
    None,
...
def main(_):
    ...
    def get_dataset(is_training, config):
        file_pattern = (
            FLAGS.training_file_pattern if is_training else FLAGS.val_file_pattern
        )
        if not file_pattern:
