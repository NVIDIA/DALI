...
class ServingDriver:
    ...
    def visualize(self, image, boxes, classes, scores, **kwargs):
        ...
        return visualize_image(
            image, boxes, classes.astype(int), scores, self.label_map, **kwargs
        )
    ...
    def _get_model_and_spec(self):
        ...
        if self.only_network:
            ...
            spec = tf.TensorSpec(
                shape=[self.batch_size, *image_size, 3], dtype=tf.float32, name="images"
            )
    ...
    def export(
        self, output_dir: Text = None, tensorrt: Text = None, tflite: Text = None
    ):
