...
class EvaluationMetric:
        ...
        if self.testdev_dir:
                ...
                box_result_list.append(
                    {
                        ...
                        "bbox": np.around(
                            det[1:5].astype(np.float64), decimals=2
                        ).tolist(),
            ...
            if self.label_map:
                ...
                for c in range(precision.shape[-1]):  # iterate over all classes
                    ...
                    ap_c = np.mean(precision_c) if precision_c.size else -1.0
                    ap_perclass[c] = ap_c
    ...
    def estimator_metric_fn(self, detections, groundtruth_data):
        ...
        with tf.name_scope("coco_metric"):
            if self.testdev_dir:
                update_op = tf.numpy_function(
                    self.update_state, [groundtruth_data, detections], []
                )
                ...
                if self.label_map:
                    ...
                    for i, cid in enumerate(sorted(label_map.keys())):
                        ...
                        metrics_dict[name] = (
                            metrics[i + len(self.metric_names)],
                            update_op,
                        )
                return metrics_dict
