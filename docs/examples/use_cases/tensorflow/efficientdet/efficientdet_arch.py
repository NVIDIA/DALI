...
def freeze_vars(variables, pattern):
    ...
    if pattern:
        ...
        if len(filtered_vars) == len(variables):
            logging.warning(
                "%s didnt match with any variable. Please use compatible "
                'pattern. i.e "(efficientnet)"',
                pattern,
            )
...
def build_backbone(features, config):
    ...
    if "efficientnet" in backbone_name:
        ...
        if config.backbone_config is not None:
            override_params["blocks_args"] = efficientnet_builder.BlockDecoder().encode(
...
def build_bifpn_layer(feats, feat_sizes, config):
    ...
    if p.fpn_config:
        ...
        fpn_config = fpn_configs.get_fpn_config(
...
def efficientdet(features, model_name=None, config=None, **kwargs):
    ...
    logging.info(
        "backbone params/flops = {:.6f}M, {:.9f}B".format(*utils.num_params_flops())
    )

    # build feature network.
    fpn_feats = build_feature_network(features, config)
    logging.info(
        "backbone+fpn params/flops = {:.6f}M, {:.9f}B".format(*utils.num_params_flops())
    )

    # build class and box predictions.
    class_outputs, box_outputs = build_class_and_box_outputs(fpn_feats, config)
    logging.info(
        "backbone+fpn+box params/flops = {:.6f}M, {:.9f}B".format(
