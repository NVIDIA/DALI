...
def _get_v(
    b1_height: FloatType, b1_width: FloatType, b2_height: FloatType, b2_width: FloatType
) -> tf.Tensor:
...
def _iou_per_anchor(
    pred_boxes: FloatType, target_boxes: FloatType, iou_type: Text = "iou"
) -> tf.Tensor:
...
def iou_loss(
    ...
    for i in range(0, len(pred_boxes_list), 4):
        ...
        iou_loss_list.append(
            mask * (1 - tf.squeeze(_iou_per_anchor(pred_boxes, target_boxes, iou_type)))
        )
