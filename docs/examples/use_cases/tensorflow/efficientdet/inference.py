...
def batch_image_preprocess(
    raw_images, image_size: Union[int, Tuple[int, int]], batch_size: int = None
):
...
def visualize_image_prediction(image, prediction, label_map=None, **kwargs):
...
class ServingDriver(object):
    ...
    def visualize(self, image, prediction, **kwargs):
        ...
        return visualize_image_prediction(
            image, prediction, label_map=self.label_map, **kwargs
        )
    ...
    def export(self, output_dir: Text, tflite_path: Text = None, tensorrt: Text = None):
...
class InferenceDriver(object):
    ...
    def __init__(
        self, model_name: Text, ckpt_path: Text, model_params: Dict[Text, Any] = None
    ):
        """Initialize the inference driver.
        ...
        with tf.Session() as sess:
            ...
            for i, prediction in enumerate(predictions):
                img = visualize_image_prediction(
                    raw_images[i], prediction, label_map=self.label_map, **kwargs
                )
