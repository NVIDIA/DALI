...
class IouUtilsTest(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        self.pb = tf.constant(
            [[4.0, 3.0, 7.0, 5.0], [5.0, 6.0, 10.0, 7.0]], dtype=tf.float32
        )
        self.tb = tf.constant(
    ...
    def test_iou(self):
        self.assertAllClose(iou_utils.iou_loss(self.pb, self.tb, "iou"), [0.875, 1.0])
    ...
    def test_iou_zero_target(self):
        self.assertAllClose(iou_utils.iou_loss(self.pb, self.zeros, "iou"), [0.0, 0.0])
        self.assertAllClose(iou_utils.iou_loss(self.pb, self.zeros, "ciou"), [0.0, 0.0])
        self.assertAllClose(iou_utils.iou_loss(self.pb, self.zeros, "diou"), [0.0, 0.0])
        self.assertAllClose(iou_utils.iou_loss(self.pb, self.zeros, "giou"), [0.0, 0.0])
