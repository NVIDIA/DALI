...
os.system(f"docker run --network=internal --gpus all --shm-size=1g --ulimit memlock=-1 --ulimit stack=67108864 --rm -v {args.logs}:/dlogs -v {args.coco}:/coco -v {args.data}:/data --name {args.name} {args.tag} bash -c \"python3 main.py {args.cmd} 2>&1 | tee -a /dlogs/output.log\"")
