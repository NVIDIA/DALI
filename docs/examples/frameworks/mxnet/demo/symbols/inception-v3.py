...
def Inception7A(
    ...
    tower_5x5 = Conv(
        data, num_5x5_red, name=("%s_tower" % name), suffix="_conv"
    )
    tower_5x5 = Conv(
        tower_5x5,
        num_5x5,
    ...
    tower_3x3 = Conv(
        data, num_3x3_red, name=("%s_tower_1" % name), suffix="_conv"
    )
    tower_3x3 = Conv(
        tower_3x3,
        num_3x3_1,
...
# First Downsample
def Inception7B(
    data, num_3x3, num_d3x3_red, num_d3x3_1, num_d3x3_2, pool, name
):
    tower_3x3 = Conv(
        data,
        num_3x3,
    ...
    tower_d3x3 = Conv(
        data, num_d3x3_red, name=("%s_tower" % name), suffix="_conv"
    )
    tower_d3x3 = Conv(
        tower_d3x3,
        num_d3x3_1,
...
def Inception7C(
    ...
    tower_1x1 = Conv(
        data=data, num_filter=num_1x1, kernel=(1, 1), name=("%s_conv" % name)
    )
    tower_d7 = Conv(
        data=data,
...
def Inception7D(
    ...
    # concat
    concat = mx.sym.Concat(
        *[tower_3x3, tower_d7_3x3, pooling], name="ch_concat_%s_chconcat" % name
    )
    return concat
...
def get_symbol(num_classes=1000, dtype="float32", **kwargs):
    ...
    pool = mx.sym.Pooling(
        data=conv_2, kernel=(3, 3), stride=(2, 2), pool_type="max", name="pool"
    )
    # stage 2
    conv_3 = Conv(pool, 80, kernel=(1, 1), name="conv_3")
    conv_4 = Conv(conv_3, 192, kernel=(3, 3), name="conv_4")
    pool1 = mx.sym.Pooling(
        data=conv_4, kernel=(3, 3), stride=(2, 2), pool_type="max", name="pool1"
    )
    # stage 3
    ...
    # stage 4
    in4a = Inception7C(
        in3d, 192, 128, 128, 192, 128, 128, 128, 128, 192, "avg", 192, "mixed_4"
    )
    in4b = Inception7C(
        in4a, 192, 160, 160, 192, 160, 160, 160, 160, 192, "avg", 192, "mixed_5"
    )
    in4c = Inception7C(
        in4b, 192, 160, 160, 192, 160, 160, 160, 160, 192, "avg", 192, "mixed_6"
    )
    in4d = Inception7C(
        in4c, 192, 192, 192, 192, 192, 192, 192, 192, 192, "avg", 192, "mixed_7"
    )
    in4e = Inception7D(in4d, 192, 320, 192, 192, 192, 192, "max", "mixed_8")
    # stage 5
    in5a = Inception7E(
        in4e, 320, 384, 384, 384, 448, 384, 384, 384, "avg", 192, "mixed_9"
    )
    in5b = Inception7E(
        in5a, 320, 384, 384, 384, 448, 384, 384, 384, "max", 192, "mixed_10"
    )
    # pool
    pool = mx.sym.Pooling(
        data=in5b,
