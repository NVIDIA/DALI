...
def ConvFactory(
    data, num_filter, kernel, stride=(1, 1), pad=(0, 0), name=None, suffix=""
):
    ...
    act = mx.symbol.Activation(
        data=conv, act_type="relu", name="relu_%s%s" % (name, suffix)
    )
    return act


def InceptionFactory(
    data, num_1x1, num_3x3red, num_3x3, num_d5x5red, num_d5x5, pool, proj, name
):
    # 1x1
    c1x1 = ConvFactory(
        data=data, num_filter=num_1x1, kernel=(1, 1), name=("%s_1x1" % name)
    )
    # 3x3 reduce + 3x3
    c3x3r = ConvFactory(
        data=data,
    ...
    cproj = ConvFactory(
        data=pooling, num_filter=proj, kernel=(1, 1), name=("%s_proj" % name)
    )
    # concat
    concat = mx.symbol.Concat(
        *[c1x1, c3x3, cd5x5, cproj], name="ch_concat_%s_chconcat" % name
    )
    return concat
...
def get_symbol(num_classes=1000, **kwargs):
    ...
    conv1 = ConvFactory(
        data, 64, kernel=(7, 7), stride=(2, 2), pad=(3, 3), name="conv1"
    )
    ...
    conv3 = ConvFactory(
        conv2, 192, kernel=(3, 3), stride=(1, 1), pad=(1, 1), name="conv3"
    )
    ...
    in4e = InceptionFactory(
        in4d, 256, 160, 320, 32, 128, "max", 128, name="in4e"
    )
    pool5 = mx.sym.Pooling(in4e, kernel=(3, 3), stride=(2, 2), pool_type="max")
    in5a = InceptionFactory(
        pool5, 256, 160, 320, 32, 128, "max", 128, name="in5a"
    )
    in5b = InceptionFactory(
        in5a, 384, 192, 384, 48, 128, "max", 128, name="in5b"
    )
