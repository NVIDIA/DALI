...
def ConvFactory(data, num_filter, kernel, stride=(1, 1), pad=(0, 0), name=None, suffix="", attr={}):
    ...
    bn = mx.symbol.BatchNorm(
        data=conv, fix_gamma=fix_gamma, eps=eps, momentum=bn_mom, name="bn_%s%s" % (name, suffix)
    )
    act = mx.symbol.Activation(
        data=bn, act_type="relu", name="relu_%s%s" % (name, suffix), attr=attr
    )
    return act
...
def InceptionFactoryA(data, num_1x1, num_3x3red, num_3x3, num_d3x3red, num_d3x3, pool, proj, name):
    ...
    # 3x3 reduce + 3x3
    c3x3r = ConvFactory(
        data=data, num_filter=num_3x3red, kernel=(1, 1), name=("%s_3x3" % name), suffix="_reduce"
    )
    c3x3 = ConvFactory(
        data=c3x3r, num_filter=num_3x3, kernel=(3, 3), pad=(1, 1), name=("%s_3x3" % name)
    )
    ...
    cd3x3 = ConvFactory(
        data=cd3x3r, num_filter=num_d3x3, kernel=(3, 3), pad=(1, 1), name=("%s_double_3x3_0" % name)
    )
    cd3x3 = ConvFactory(
        data=cd3x3, num_filter=num_d3x3, kernel=(3, 3), pad=(1, 1), name=("%s_double_3x3_1" % name)
    )
    # pool + proj
...
def SimpleFactory(data, ch_1x1, ch_3x3, name, attr):
    # 1x1
    conv1x1 = ConvFactory(
        data=data, name=name + "_1x1", kernel=(1, 1), pad=(0, 0), num_filter=ch_1x1, attr=attr
    )
    # 3x3
    conv3x3 = ConvFactory(
        data=data, name=name + "_3x3", kernel=(3, 3), pad=(1, 1), num_filter=ch_3x3, attr=attr
    )
    # concat
...
def get_symbol(num_classes, image_shape, **kwargs):
    ...
    if height <= 28:
        # a simper version
        conv1 = ConvFactory(
            data=data, kernel=(3, 3), pad=(1, 1), name="1", num_filter=96, attr=attr
        )
        ...
        pool = mx.symbol.Pooling(
            data=in5b, pool_type="avg", kernel=(7, 7), name="global_pool", attr=attr
        )
    else:
        # stage 1
        conv1 = ConvFactory(
            data=data, num_filter=64, kernel=(7, 7), stride=(2, 2), pad=(3, 3), name="1"
        )
        pool1 = mx.symbol.Pooling(
            data=conv1, kernel=(3, 3), stride=(2, 2), name="pool_1", pool_type="max"
        )
        # stage 2
        conv2red = ConvFactory(
            data=pool1, num_filter=64, kernel=(1, 1), stride=(1, 1), name="2_red"
        )
        conv2 = ConvFactory(
            data=conv2red, num_filter=192, kernel=(3, 3), stride=(1, 1), pad=(1, 1), name="2"
        )
        pool2 = mx.symbol.Pooling(
            data=conv2, kernel=(3, 3), stride=(2, 2), name="pool_2", pool_type="max"
        )
        # stage 2
        ...
        # global avg pooling
        pool = mx.symbol.Pooling(
            data=in5b, kernel=(7, 7), stride=(1, 1), name="global_pool", pool_type="avg"
        )
