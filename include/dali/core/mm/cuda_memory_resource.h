...
template <class _Tp>
struct  _DALI_TEMPLATE_VIS __unique_typeinfo { static constexpr int __id = 0; };
template <class _Tp> constexpr int __unique_typeinfo<_Tp>::__id;

template <class _Tp>
inline _DALI_INLINE_VISIBILITY
...
private:
  ...
  void do_deallocate(void *__p, std::size_t __bytes,
                     std::size_t __alignment) override = 0;
...
template <typename _Kind>
inline _DALI_INLINE_VISIBILITY
bool operator==(const memory_resource<_Kind> &__a, const memory_resource<_Kind> &__b) {
...
template <> struct kind_has_property<__kind, __property> : std::true_type {};
...
namespace detail {
    ...
    template <typename _Property, typename _MemoryKind>
    kind_has_property<_MemoryKind, _Property> Has_Property(const stream_ordered_memory_resource<_MemoryKind> *);
}  // namespace detail

template <typename _Target, typename _Property>
struct has_property : decltype(detail::Has_Property<_Property>(std::declval<std::remove_reference_t<_Target>*>())) {};
...
struct is_resource_pointer_convertible<_FromPointer, detail::memory_resource_base*>
   : std::conjunction<std::is_pointer<_FromPointer>,
...
struct has_property<basic_resource_view<_Pointer, _Properties...>, _Property> : detail::is_property_in<_Property, _Properties...> {};
...
struct is_view_convertible<
 ...
 : std::conjunction<detail::is_resource_pointer_convertible<_FromPointer, _ToPointer>,
...
public:
    static_assert(
        std::is_base_of<detail::memory_resource_base, typename ::std::pointer_traits<_ResourcePointer>::element_type>::value ||
        std::is_base_of<detail::stream_ordered_memory_resource_base, typename ::std::pointer_traits<_ResourcePointer>::element_type>::value,
     ...
     */
    template <typename _Resource, typename = std::enable_if_t<
     ...
     */
    template <typename _OtherPointer, typename... _OtherProperties, typename = std::enable_if_t<
    ...
    bool operator==(const cuda_for_dali::basic_resource_view<_Ptr2, _Props2...> &__v2) const noexcept {
...
bool operator==(const basic_resource_view<_ResourcePointer, _Properties...> &__view, const memory_resource<_Kind> *__mr) {
...
template <typename _ResourcePointer, typename... _Properties, typename _Kind>
bool operator==(const memory_resource<_Kind> *__mr, const basic_resource_view<_ResourcePointer, _Properties...> &__view) {
...
public:
  virtual cuda_for_dali::memory_resource<cuda_for_dali::memory_kind::host>* resource() const noexcept = 0;
};
}
...
class pmr_adaptor final : public detail::__pmr_adaptor_base {
  ...
  static constexpr bool __is_host_accessible_resource = has_property<resource_type, memory_access::host>::value;
  static_assert(
