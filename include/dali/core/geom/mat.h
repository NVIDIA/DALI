...
struct mat {
  ...
  #define DEFINE_MAT_UNOP(op)\
  auto operator op() const {\
    mat<rows, cols, decltype(op at(0, 0))> result = {};\
    MAT_STORAGE_LOOP(i)\
      result.m[i] = op m[i];\
    return result;\
  }
  ...
  template <typename U>\
  DALI_HOST_DEV\
  std::enable_if_t<is_scalar<U>::value, mat<rows, cols, Element> &> operator op(const U &other) {\
    MAT_STORAGE_LOOP(i)\
      m[i] op other;\
    return *this;\
  }
  ...
  inline auto operator*(const mat<cols, rhs_cols, U> &rhs) const {
    using Out = decltype(dot(row(0), rhs.col(0)));
    mat<rows, rhs_cols, Out> result = {};
