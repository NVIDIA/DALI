...
class DLL_PUBLIC VideoDecoderBase {
 public:
  ...
  using OutBackend = std::conditional_t<std::is_same_v<Backend, dali::CPUBackend>,
                                        dali::CPUBackend,
                                        dali::GPUBackend>;
 ...
 protected:
  ...
  dali::TensorListShape<4> ReadOutputShape() {
    dali::TensorListShape<4> shape(frames_decoders_.size());
    for (size_t s = 0; s < frames_decoders_.size(); ++s) {
      dali::TensorShape<4> sample_shape;
   ...
   */
  bool DecodeFrames(dali::SampleView<OutBackend> output, int64_t sample_idx, int64_t num_frames,
                    std::optional<dali::SampleView<OutBackend>> pad_value = std::nullopt,
    ...
    DALI_ENFORCE(!pad_value.has_value() || pad_value->shape().num_elements() == frame_size,
                 dali::make_string("Provided pad_value has improper number of elements. Expected: ",
    ...
    for (; f < num_frames && pad_value.has_value(); f++) {
      dali::kernels::copy<storage_backend_for_copy_kernel, storage_backend_for_copy_kernel>(
              output_data + f * frame_size, pad_value->raw_data(), frame_size,
              std::is_same_v<storage_backend_for_copy_kernel, dali::StorageGPU> ? *stream : 0);
    }
   ...
   */
  void DecodeSample(dali::SampleView<OutBackend> output, int64_t idx) {
 ...
 private:
  using storage_backend_for_copy_kernel = std::conditional_t<
          std::is_same_v<OutBackend, dali::CPUBackend>,
          dali::StorageCPU,
          dali::StorageGPU
  >;
};
