...
struct PadFrameCreator {
  static constexpr bool is_cpu = std::is_same_v<OutBackend, dali::CPUBackend>;
  ...
  PadFrameCreator(PadType value, dali::TensorShape<3> frame_shape,
                  std::optional<cudaStream_t> stream = std::nullopt) {
    Initialize(value, frame_shape, stream);
  }


  void Initialize(PadType value, dali::TensorShape<3> frame_shape,
                  std::optional<cudaStream_t> stream = std::nullopt) {
    pad_frame_data_ = std::vector<PadType>(frame_shape.num_elements(), value);
    pad_frame_.Resize(frame_shape, dali::TypeTable::GetTypeId<PadType>());
    if (stream) {
  ...
  /// Pad frame.
  dali::Tensor<OutBackend> pad_frame_;


  dali::SampleView<OutBackend> GetPadFrame() {
...
using frames_decoder_t =
        std::conditional_t<
                std::is_same<Backend, dali::CPUBackend>::value,
                FramesDecoder,
                FramesDecoderGpu
        >;
...
class VideoInput : public VideoDecoderBase<Backend, FramesDecoder>, public dali::InputOperator<Backend> {
 public:
  static constexpr bool is_cpu = std::is_same_v<Backend, dali::CPUBackend>;
  using InBackend = dali::CPUBackend;
  using OutBackend = std::conditional_t<is_cpu, dali::CPUBackend, dali::GPUBackend>;
  static_assert((is_cpu && std::is_same_v<FramesDecoder, FramesDecoder>) ||
  ...
  explicit VideoInput(const dali::OpSpec &spec) :
          dali::InputOperator<Backend>(spec),
    ...
    DALI_ENFORCE(last_sequence_policy_ == "partial" || last_sequence_policy_ == "pad",
                 dali::make_string("Provided `last_sequence_policy` is not supported: ",
  ...
  dali::DALIDataType in_dtype() const override {
    return dali::DALIDataType::DALI_UINT8;
  }
   ...
   */
  void VideoInputRunImpl(dali::Workspace &ws);
 ...
 private:
  ...
  void LoadDataFromInputOperator(dali::ThreadPool &thread_pool);

  void SetNextDataIdTrace(dali::Workspace &ws, std::string next_data_id);

  void CreateDecoder(const dali::Workspace &ws);
  ...
  dali::TensorShape<4> GetSequenceShape(int frames_per_sequence, int input_sample_idx) {
    dali::TensorShape<4> ret;
  ...
  dali::TensorShape<3> GetFrameShape(int input_sample_idx) {
    dali::TensorShape<3> ret;
  ...
  dali::ThreadPool &GetThreadPool(const dali::Workspace &ws) {
    if constexpr (is_cpu) {
  ...
  dali::TensorList<InBackend> encoded_video_;
  ...
  std::deque<dali::OutputDesc> output_descs_;
  ...
  std::optional<dali::ThreadPool> thread_pool_ = std::nullopt;
 ...
 */
inline bool IsVideoInput(const dali::OpSchema& schema) {
  return schema.name() == "experimental__inputs__Video";
}


template<typename Backend, typename FramesDecoder>
void VideoInput<Backend, FramesDecoder>::LoadDataFromInputOperator(dali::ThreadPool &thread_pool) {
  ...
  encoded_video_.set_pinned(device_id_ != dali::CPU_ONLY_DEVICE_ID);
...
template<typename Backend, typename FramesDecoder>
void
VideoInput<Backend, FramesDecoder>::SetNextDataIdTrace(dali::Workspace &ws, std::string next_data_id) {
  ws.SetOperatorTrace(next_output_data_id_trace_name_, std::move(next_data_id));
}


template<typename Backend, typename FramesDecoder>
bool VideoInput<Backend, FramesDecoder>::SetupImpl(std::vector<dali::OutputDesc> &output_desc,
                                                   const dali::Workspace &ws) {
  if (!initialized_) {
    if (needs_data_load_) {
      dali::InputOperator<Backend>::HandleDataAvailability();
...
template<typename Backend, typename FramesDecoder>
void VideoInput<Backend, FramesDecoder>::VideoInputRunImpl(dali::Workspace &ws) {
  auto &output = ws.Output<OutBackend>(0);
  output.SetLayout("FHWC");

  bool full_sequence;
  for (int64_t s = 0; s < output.num_samples(); s++) {
    auto pad_value = last_sequence_policy_ == "pad" ? std::optional<dali::SampleView<OutBackend>>(
  ...
  dali::InputOperator<Backend>::SetDepletedOperatorTrace(ws, !will_return_next);
}
