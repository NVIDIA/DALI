...
template<>
void VideoInput<dali::CPUBackend, FramesDecoder>::CreateDecoder(const dali::Workspace &ws) {
  ...
  this->frames_decoders_[0] = std::make_unique<FramesDecoder>(data, size, false);
}
...
class VideoInputCpu : public VideoInput<dali::CPUBackend> {
  /*
   ...
   */
 public:
  explicit VideoInputCpu(const dali::OpSpec &spec) : VideoInput<dali::CPUBackend>(spec) {}
  void RunImpl(dali::Workspace &ws) override { VideoInputRunImpl(ws); }
};
