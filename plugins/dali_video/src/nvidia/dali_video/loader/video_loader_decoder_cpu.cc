...
void VideoLoaderDecoderCpu::PrepareEmpty(VideoSample<dali::CPUBackend> &sample) {
  sample = {};
  sample.data_.set_pinned(false);
  sample.data_.SetLayout("FHWC");
}

void VideoLoaderDecoderCpu::ReadSample(VideoSample<dali::CPUBackend> &sample) {
  ...
  sample.data_.Resize(
    dali::TensorShape<4>{sequence_len_, video_file.Height(), video_file.Width(), video_file.Channels()},
    dali::DALIDataType::DALI_UINT8);
...
dali::Index VideoLoaderDecoderCpu::SizeImpl() {
...
void VideoLoaderDecoderCpu::Reset(bool wrap_to_shard) {
  current_index_ = wrap_to_shard ? dali::start_index(virtual_shard_id_, num_shards_, SizeImpl()) : 0;
}
