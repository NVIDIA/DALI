...
void FramesDecoder::InitAvState(bool init_codecs) {
  ...
  DALI_ENFORCE(
    ret >= 0,
    dali::make_string("Could not fill the codec based on parameters: ", detail::av_error_string(ret)));
  ...
  if (init_codecs) {
    ...
    DALI_ENFORCE(
      ret == 0,
      dali::make_string("Could not initialize codec context: ", detail::av_error_string(ret)));
...
bool FramesDecoder::FindVideoStream(bool init_codecs) {
  if (init_codecs) {
    ...
    if (i >= av_state_->ctx_->nb_streams) {
      DALI_WARN(dali::make_string("Could not find a valid video stream in a file ", Filename()));
      return false;
    }
  } else {
  ...
  if (Height() == 0 || Width() == 0) {
    if (avformat_find_stream_info(av_state_->ctx_, nullptr) < 0) {
      DALI_WARN(dali::make_string("Could not find stream information in ", Filename()));
      return false;
    }
    if (Height() == 0 || Width() == 0) {
...
FramesDecoder::FramesDecoder(const std::string &filename)
  ...
  if (ret != 0) {
    DALI_WARN(dali::make_string("Failed to open video file ", Filename(), "due to ",
  ...
  if (!CheckCodecSupport()) {
    DALI_WARN(dali::make_string("Unsupported video codec: ", CodecName(),
...
void FramesDecoder::CopyToOutput(uint8_t *data) {
  ...
  DALI_ENFORCE(
    ret >= 0,
    dali::make_string("Could not convert frame data to RGB: ", detail::av_error_string(ret)));
}
...
bool FramesDecoder::ReadRegularFrame(uint8_t *data, bool copy_to_output) {
  ...
  while (av_read_frame(av_state_->ctx_, av_state_->packet_) >= 0) {
    ...
    DALI_ENFORCE(
      ret >= 0,
      dali::make_string("Failed to send packet to decoder: ", detail::av_error_string(ret)));
...
void FramesDecoder::SeekFrame(int frame_id) {
  ...
  DALI_ENFORCE(
    ...
    dali::make_string("Invalid seek frame id. frame_id = ", frame_id, ", num_frames = ", NumFrames()));
