...
NumpyHeaderMeta ParseHeader(InputStream *src) {
  NumpyHeaderMeta parsed_header;
...
DALIDataType NumpyHeaderMeta::type() const {
  return type_info ? type_info->id() : DALI_NO_TYPE;
}

size_t NumpyHeaderMeta::size() const {
  return volume(shape);
}

size_t NumpyHeaderMeta::nbytes() const {
