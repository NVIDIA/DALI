...
// libraries.
void RunInitOrShutdown(std::function<void(int)> work) {
...
S3ClientManager& S3ClientManager::Instance() {
  static int init = [&]() {
    RunInitOrShutdown([&](int) { Aws::InitAPI(Aws::SDKOptions{}); });
    return 0;
  }();
  // We want RunInitOrShutdown's thread pool to outlive s_manager_
