...
TEST_F(FileReaderTest, SimpleCheckpointing) {
  ...
  for (int i = 0; i < iterations; i++) {
    auto [res, cpt] = CheckpointIteration(pipe);
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    EXPECT_EQ(RunIteration(fresh_pipe), results[i]);
  }
}
