...
TEST_F(NonsilenceOpTest, LeadTrailThreshTest) {
  ...
  EXPECT_EQ(NonsilenceOperatorCpu::Impl::LeadTrailThresh(make_cspan(t3), 100),
            std::make_pair(0, 0));
  ...
  EXPECT_EQ(NonsilenceOperatorCpu::Impl::LeadTrailThresh(make_cspan(t5), 1), std::make_pair(0, 0));
