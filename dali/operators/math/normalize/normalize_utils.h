...
/**
 *  @brief Calculates scale/sqrt(x) for nonzero x and keeps 0 when x == 0
 */
static void ScaleRSqrtKeepZero(float *data, int64_t n, float scale) {
  int i = 0;
  ...
  for (; i < n; i++)
    if (data[i])
      data[i] = rsqrt(data[i] * scale);
}

static void ScaleRSqrtKeepZero(const TensorView<StorageCPU, float> &inout, float scale) {
  ScaleRSqrtKeepZero(inout.data, inout.num_elements(), scale);
}

static void SumSquare2InvStdDev(const TensorView<StorageCPU, float> &inout,
                                const TensorShape<> &data_shape,
                                double scale) {
  scale = scale * inout.num_elements() / data_shape.num_elements();
  ScaleRSqrtKeepZero(inout, static_cast<float>(scale));
}
