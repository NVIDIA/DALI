      ...
      VALUE_SWITCH(dim_desc.usable_axes_count, Axes, LAPLACIAN_SUPPORTED_AXES, (
        BOOL_SWITCH(dim_desc.is_channel_last(), HasChannels, (
          BOOL_SWITCH(dim_desc.is_sequence(), IsSeq, (
            if (dtype == input.type()) {
              using LaplacianSame = laplacian::LaplacianOpGpu<In, In, Axes, HasChannels, IsSeq>;
              impl_ = std::make_unique<LaplacianSame>(spec_, dim_desc);
            } else {
              using LaplacianFloat = laplacian::LaplacianOpGpu<float, In, Axes, HasChannels, IsSeq>;
