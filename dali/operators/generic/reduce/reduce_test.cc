...
void ReduceVerify(TensorListWrapper input, TensorListWrapper output, Arguments args) {
    ...
    for (size_t i = 0; i < output_d->ntensor(); ++i) {
        auto out_tensor = output_d->tensor<int>(i);
        cout << out_tensor[0] << "    ";
    }
    cout << endl;
}
...
input_t PrepareInput() {
    ...
    input->set_type(TypeTable::GetTypeInfo(DALIDataType::DALI_INT32));
    ...
    for (int sample = 0; sample < batch_size; ++sample) {
        auto sample_data = input->mutable_tensor<int>(sample);
...
TEST_F(ReduceTest, Simple) {
    input_t input = PrepareInput();
    
    TensorListWrapper output;
    this->RunTest(input.get(), output, Arguments {}, ReduceVerify);
}
