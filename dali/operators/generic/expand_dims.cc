...
DALI_SCHEMA(ExpandDims)
  .DocStr("Insert new dimension[s] of extent 1 and inserts new entries in "
    "the layout (new_axis_names) at these indices in the layout.") // TODO CAŁA SCHEMA
  .NumInput(1, 1)
  .NumOutput(1)
  ...
  .AddOptionalArg("new_axis_names", "", TensorLayout(""));
...
void ExpandDims<Backend>::GenerateSrcDims(const Workspace &ws) {
  ...
  for (int d = 0; d < ndim; d++) {
    if (axes_[axes_ind] == d) {
      ...
      out_layout.push_back(new_axis_names_.empty() ? '?' : new_axis_names_[axes_ind]);
      axes_ind++;
    }

    out_layout.push_back(in_layout.empty() ? '?' : in_layout[d]);
