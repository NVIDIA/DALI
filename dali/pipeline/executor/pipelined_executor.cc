  ...
  for (int i = 0; i < graph_->NumOp(DALIOpType::SUPPORT); ++i) {
    ...
    for (int j = 0; j < node.spec.NumOutput(); ++j) {
      ...
      for (auto &meta : consumer_meta) {
        ...
        if (node_type != DALIOpType::SUPPORT && node_type != DALIOpType::CPU) {
    ...
    for (int j = 0; j < node.spec.NumOutput(); ++j) {
      ...
      for (auto &meta : consumer_meta) {
        ...
        if (type != DALIOpType::CPU) {
          ...
          has_gpu_consumer =
              has_gpu_consumer || type == DALIOpType::GPU ||
...
void PipelinedExecutor::SetStageOutputsForIter(int queue_idx, WorkspaceBlob *wsb) {
  ...
  for (size_t i = 0; i < support_stage_outputs_.size(); ++i) {
    ...
    get_workspace<DALIOpType::SUPPORT>(wsb->op_data, support_op_id)
    ...
    for (size_t j = 0; j < info.con_and_idx.size(); ++j) {
      ...
      if (graph_->NodeType(node_id) == DALIOpType::MIXED) {
        get_workspace<DALIOpType::MIXED>(wsb->op_data, child_op_id)
            .SetArgumentInput(tvp.Get(queue_idx), arg_name);
      } else if (graph_->NodeType(node_id) == DALIOpType::GPU) {
        get_workspace<DALIOpType::GPU>(wsb->op_data, child_op_id)
            .SetArgumentInput(tvp.Get(queue_idx), arg_name);
      } else {
        get_workspace<DALIOpType::CPU>(wsb->op_data, child_op_id)
            .SetArgumentInput(tvp.Get(queue_idx), arg_name);
      }
    }
  }
  ...
  for (size_t i = 0; i < cpu_stage_outputs_.size(); ++i) {
    ...
    int output_idx = info.prod_and_idx.second;
    get_workspace<DALIOpType::CPU>(wsb->op_data, cpu_op_id)
        .SetOutput(output_idx, tvp.Get(queue_idx));
