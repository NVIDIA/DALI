...
class ArgValue {
 public:
  static_assert(ndim == 0 || ndim == 1,
    "Only Scalar and 1D arguments are supported at the moment");

  using TLV = TensorListView<StorageCPU, const T, ndim>;
  using TV = TensorView<StorageCPU, const T, ndim>;

  ArgValue(const std::string &arg_name, const OpSpec &spec)
      : arg_name_(arg_name),
  ...
  void Acquire(const OpSpec &spec, const ArgumentWorkspace &ws, int nsamples = 1,
    ...
    if (has_arg_input_) {
      ...
      std::vector<const T*> ptrs(nsamples, nullptr);
      for (int i = 0; i < nsamples; i++)
        ptrs[i] = data_.data();
      view_ = TLV(std::move(ptrs), uniform_list_shape(nsamples, shape));
    }
  }
