...
class ExpandDescFrameInfoFn {
 public:
  inline ExpandDescFrameInfoFn(const ExpandDesc *expand_desc)  // NOLINT
...
inline TensorListShape<> fold_outermost_like(const TensorListShape<> &shape,
  ...
  for (int i = 0; i < num_samples; i++) {
    const auto &element_shape = shape[element_offset];
    auto group_shape = folded_shape[i];
    res.set_tensor_shape(i, shape_cat(group_shape, element_shape));
    auto num_elements = volume(group_shape);
    for (int j = 1; j < num_elements; j++) {
      DALI_ENFORCE(shape[element_offset + j] == element_shape);
    }
