...
TEST(BoundingBoxTest, BoundingBoxLTRBDoesNotAcceptNegativeCoordinates) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{-0.5, 0.5, 0.5, 0.5}));
}
...
TEST(BoundingBoxTest, BoundingBoxLTRBDoesNotAcceptCoordinatesLargerThanOne) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{1.1, 0.5, 0.5, 0.5}));
}
...
TEST(BoundingBoxTest, BoundingBoxLTRBDoesNotAcceptCoordinatesLargerThanOne2) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.5, 1.1, 0.5, 0.5}));
}
...
TEST(BoundingBoxTest, BoundingBoxLTRBDoesNotAcceptCoordinatesLargerThanOne3) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.5, 0.5, 1.1, 0.5}));
}
...
TEST(BoundingBoxTest, BoundingBoxLTRBDoesNotAcceptCoordinatesLargerThanOne4) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.5, 0.5, 0.5, 1.1}));
}
...
TEST(BoundingBoxTest, BoundingBoxRightMustBeGreaterOrEqualToLeft) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.5, 0.0, 0.0, 0.0}));
}

TEST(BoundingBoxTest, BoundingBoxTopMustBeGreaterOrEqualToBottom) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.5, 0.0, 0.0}));
}

TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptNegativeCoordinates) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{-0.5, 0.5, 0.5, 0.5}));
}
...
TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptNegativeCoordinates3) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.5, 0.0, -0.6, 0.0}));
}

TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptNegativeCoordinates4) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.5, 0.0, -0.6}));
}

TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptCoordinatesLargerThanOne) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{1.1, 0.5, 0.5, 0.5}));
}

TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptCoordinatesLargerThanOne2) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.5, 1.1, 0.5, 0.5}));
}

TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptCoordinatesLargerThanOne3) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.5, 0.0, 1.1, 0.0}));
}

TEST(BoundingBoxTest, BoundingBoxXYWHDoesNotAcceptCoordinatesLargerThanOne4) {
  EXPECT_ANY_THROW(BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.5, 0.0, 1.1}));
}
...
TEST(BoundingBoxTest, ContainsReturnsTrueIfPointIsContained) {
  auto box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.25, 0.25, 0.75, 0.75});

  EXPECT_TRUE(box.Contains(std::array<float, 2>{0.5, 0.5}));
  EXPECT_TRUE(box.Contains(std::array<float, 2>{0.25, 0.25}));
  EXPECT_TRUE(box.Contains(std::array<float, 2>{0.25, 0.75}));
  EXPECT_TRUE(box.Contains(std::array<float, 2>{0.75, 0.75}));

  auto box2 = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 0.0, 0.0});
  EXPECT_TRUE(box2.Contains(std::array<float, 2>{0.0, 0.0}));
}

TEST(BoundingBoxTest, ContainsReturnsTrueIfPointIsContainedXYWH) {
  auto box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.25, 0.25, 0.5, 0.5});
  ...
  auto box2 = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 0.0, 0.0});
  EXPECT_TRUE(box2.Contains(std::array<float, 2>{0.0, 0.0}));
}

TEST(BoundingBoxTest, ContainsReturnsFalseIfPointIsContained) {
  auto box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 0.0, 0.0});

  EXPECT_FALSE(box.Contains(std::array<float, 2>{0.5, 0.5}));
  EXPECT_FALSE(box.Contains(std::array<float, 2>{0.25, 0.25}));
  EXPECT_FALSE(box.Contains(std::array<float, 2>{0.25, 0.75}));
  EXPECT_FALSE(box.Contains(std::array<float, 2>{0.75, 0.75}));
}

TEST(BoundingBoxTest, ContainsReturnsFalseIfPointIsContainedXYWH) {
  auto box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 0.0, 0.0});
...
TEST(BoundingBoxTest, CanBeIntersectedToSmaller) {
  auto bigger_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 1.0, 1.0});
  auto smaller_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 0.1, 0.1});
...
TEST(BoundingBoxTest, CanFlipHorizontallyXYWH) {
  ...
  auto box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.25, 0.25, 0.25, 0.25});
...
TEST(BoundingBoxTest, CanFlipHorizontallyLTRB) {
  auto box_initial = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.25, 0.25, 0.5, 0.5});
  auto coords_initial_ltrb = box_initial.AsStartAndEnd();
  auto coords_initial_xywh = box_initial.AsStartAndShape();

  auto box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.25, 0.25, 0.5, 0.5});
...
TEST(BoundingBoxTest, CanCalculateArea) {
  auto ltrb_box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 0.0, 0.0});
  auto xywh_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 0.0, 0.0});

  EXPECT_EQ(ltrb_box.Volume(), 0.0f);
  EXPECT_EQ(xywh_box.Volume(), 0.0f);

  ltrb_box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 1.0, 1.0});
  xywh_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 1.0, 1.0});

  EXPECT_EQ(ltrb_box.Volume(), 1.0f);
  EXPECT_EQ(xywh_box.Volume(), 1.0f);

  ltrb_box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 1.0, 0.5});
  xywh_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 1.0, 0.5});

  EXPECT_EQ(ltrb_box.Volume(), 0.5f);
  EXPECT_EQ(xywh_box.Volume(), 0.5f);
}
...
TEST(BoundingBoxTest, CanFindContained) {
  ...
  EXPECT_TRUE(ltrb_box.Contains(std::array<float, 2>{0, 0}));
  EXPECT_TRUE(xywh_box.Contains(std::array<float, 2>{0, 0}));

  EXPECT_FALSE(ltrb_box.Contains(std::array<float, 2>{0.1, 0}));
  EXPECT_FALSE(xywh_box.Contains(std::array<float, 2>{0.1, 0}));

  ltrb_box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 1.0, 1.0});
  xywh_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 1.0, 1.0});

  EXPECT_TRUE(ltrb_box.Contains(std::array<float, 2>{1, 1}));
  EXPECT_TRUE(xywh_box.Contains(std::array<float, 2>{1, 1}));

  EXPECT_TRUE(ltrb_box.Contains(std::array<float, 2>{1, 0.9}));
  EXPECT_TRUE(xywh_box.Contains(std::array<float, 2>{1, 0.9}));

  ltrb_box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 1.0, 0.5});
  xywh_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.0, 0.0, 1.0, 0.5});

  EXPECT_TRUE(ltrb_box.Contains(std::array<float, 2>{1, 0.5}));
  EXPECT_TRUE(xywh_box.Contains(std::array<float, 2>{1, 0.5}));
}
...
TEST(BoundingBoxTest, CanFindIfOverlap) {
  ...
  ltrb_box = BoundingBox<2>::FromStartAndEnd(std::array<float, 4>{0.0, 0.0, 0.5, 0.5});
  xywh_box = BoundingBox<2>::FromStartAndShape(std::array<float, 4>{0.5, 0.5, 0.5, 0.5});
...
TEST(BoundingBoxTest, Bbox3dIoUOverlap) {
  auto bbox1 = BoundingBox<3>::FromStartAndEnd(std::array<float, 6>{0.0, 0.0, 0.0, 0.3, 0.3, 0.3});
  auto bbox2 = BoundingBox<3>::FromStartAndEnd(std::array<float, 6>{0.2, 0.2, 0.2, 0.5, 0.5, 0.5});
...
TEST(BoundingBoxTest, Bbox3dBboxFormatConversion) {
  std::array<float, 6> start_and_end = {0.2, 0.2, 0.2, 0.7, 0.7, 0.7};
  auto bbox1 = BoundingBox<3>::FromStartAndEnd(start_and_end);

  std::array<float, 6> start_and_shape = {0.2, 0.2, 0.2, 0.5, 0.5, 0.5};
  auto bbox2 = BoundingBox<3>::FromStartAndShape(start_and_shape);

  EXPECT_EQ(bbox1, bbox2);
  EXPECT_EQ(bbox1.AsStartAndShape(), bbox2);
  EXPECT_EQ(bbox2.AsStartAndEnd(), bbox1);
}

TEST(BoundingBoxTest, LayoutConversionStartAndEnd) {
  std::array<float, 6> start_and_end_yzx = {0.2, 0.3, 0.1, 0.44, 0.66, 0.22};
  std::array<float, 6> start_and_end_xyz = {0.1, 0.2, 0.3, 0.22, 0.44, 0.66};
  std::array<float, 6> start_and_end_zyx = {0.3, 0.2, 0.1, 0.66, 0.44, 0.22};

  ASSERT_EQ(BoundingBox<3>::FromStartAndEnd(start_and_end_yzx, "yzxYZX"),
            BoundingBox<3>::FromStartAndEnd(start_and_end_zyx, "zyxZYX"));

  ASSERT_EQ(BoundingBox<3>::FromStartAndEnd(start_and_end_yzx, "yzxYZX"),
            BoundingBox<3>::FromStartAndEnd(start_and_end_xyz, "xyzXYZ"));
...
TEST(BoundingBoxTest, LayoutConversionStartAndShape) {
  std::array<float, 6> start_and_shape_yzx = {0.2, 0.3, 0.1, 0.44, 0.66, 0.22};
  std::array<float, 6> start_and_shape_xyz = {0.1, 0.2, 0.3, 0.22, 0.44, 0.66};
  std::array<float, 6> start_and_shape_zyx = {0.3, 0.2, 0.1, 0.66, 0.44, 0.22};

  ASSERT_EQ(BoundingBox<3>::FromStartAndShape(start_and_shape_yzx, "yzxHDW"),
            BoundingBox<3>::FromStartAndShape(start_and_shape_zyx, "zyxDHW"));

  ASSERT_EQ(BoundingBox<3>::FromStartAndShape(start_and_shape_yzx, "yzxHDW"),
            BoundingBox<3>::FromStartAndShape(start_and_shape_xyz, "xyzWHD"));
...
TEST(BoundingBoxTest, From) {
  std::array<float, 6> start_and_shape_yzx = {0.2, 0.3, 0.1, 0.44, 0.66, 0.22};
  std::array<float, 6> start_and_end_yzx = {0.2, 0.3, 0.1, 0.64, 0.96, 0.32};

  ASSERT_EQ(BoundingBox<3>::FromStartAndShape(start_and_shape_yzx, "yzxHDW"),
            BoundingBox<3>::From(start_and_shape_yzx, "yzxHDW"));

  ASSERT_EQ(BoundingBox<3>::FromStartAndEnd(start_and_end_yzx, "yzxXYZ"),
            BoundingBox<3>::From(start_and_end_yzx, "yzxXYZ"));

  ASSERT_EQ(BoundingBox<3>::FromStartAndShape(start_and_shape_yzx, "yzxHDW"),
            BoundingBox<3>::FromStartAndEnd(start_and_end_yzx, "yzxYZX"));
}
