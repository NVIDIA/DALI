...
class ExpressionImplParam {
 protected:
  ...
  std::enable_if_t<IsTensor && is_cpu, const Type *> ObtainInput(const ExprNode &expr,
  ...
  template <bool IsTensor, typename Type>
  std::enable_if_t<!IsTensor, Type> ObtainInput(const ExprNode &expr,
...
std::unique_ptr<ExpressionImplBase> ExprImplFactory(const HostWorkspace &ws,
                                                    const ExprNode &expr);
...
struct ExprImplCache {
  template <typename Backend>
  ExpressionImplBase *GetExprImpl(const ExprNode &expr) {
    ...
    auto ptr = std::shared_ptr<ExpressionImplBase>(std::move(new_impl));
    cache_[node_desc] = ptr;
    return ptr.get();
  }

 private:
  std::map<std::string, std::shared_ptr<ExpressionImplBase>> cache_;
};
