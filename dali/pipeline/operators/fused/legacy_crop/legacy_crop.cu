...
template <>
template <typename Out>
void LegacyCrop<GPUBackend>::RunHelper(Workspace<GPUBackend> *ws, const int idx) {
...
template <>
void LegacyCrop<GPUBackend>::SetupSharedSampleParams(DeviceWorkspace *ws) {
...
template <>
void LegacyCrop<GPUBackend>::DataDependentSetup(DeviceWorkspace *ws, const int idx) {
...
template <>
void LegacyCrop<GPUBackend>::RunImpl(DeviceWorkspace *ws, const int idx) {
...
// Register operator
DALI_REGISTER_OPERATOR(LegacyCrop, LegacyCrop<GPUBackend>, GPU);
