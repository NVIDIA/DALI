 ...
 public:
  ...
  void RunImpl(SampleWorkspace *ws, const int idx) {
    ...
    if (ret == NVJPEG_STATUS_BAD_JPEG) {
      ...
      if (info->nvjpeg_support) {
        if (ShouldBeHybrid(*info)) {
          state_nvjpeg->nvjpeg_backend = NVJPEG_BACKEND_GPU_HYBRID;
        } else {
          state_nvjpeg->nvjpeg_backend = NVJPEG_BACKEND_HYBRID;
        }
    ...
    if (!info->nvjpeg_support) {
      ...
      out.Resize({info->heights[0], info->heights[1], GetOutputPitch(output_image_type_)});
