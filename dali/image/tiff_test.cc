...
class MyBufferTest : public DALITest {
protected:
    std::string bin = {'\x41', '\x42', '\x43', '\x44', '\x45', '\x46', '\x47', '\x48', '\x49', '\x4a', '\x4b', '\x4c',
                       '\x4d', '\x4e', '\x4f', '\x50'};
};

TEST_F(MyBufferTest, TiffBufferBigEndianTest) {
    tiff_buffer buf(bin);
...
TEST_F(MyBufferTest, TiffBufferLittleEndianTest) {
...
TEST_F(MyBufferTest, TiffBufferOffsetTest) {
