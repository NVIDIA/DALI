...
if not initialized:
    ...
    if sys.version_info[0] == 3 and sys.version_info[1] == 10:
        deprecation_warning(
            "DALI support for Python 3.10 is experimental and some functionalities "
            "may not work.")

    if __cuda_version__ < 102:
        deprecation_warning(
            "DALI 1.3 is the last official release that supports CUDA 10.0. "
...
def check_cuda_runtime():
    ...
    if not cuda_checked:
        ...
        if GetNppVersion() == -1:
            deprecation_warning(
                "Cannot access NPP library. Please check cuda installation and/or "
                "if an appropriate wheel is installed.")
