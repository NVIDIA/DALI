    ...
    for constant in code.co_consts:
        if inspect.iscode(constant):
            closure = tuple(types.CellType(None)
                            for _ in range(len(constant.co_freevars)))
            ...
            get_global_references_from_nested_code(
                constant, global_scope, global_refs)
...
def function_by_value_reducer(fun):
    ...
    get_global_references_from_nested_code(
        fun.__code__, fun.__globals__, global_refs)
    fun_context = {
...
def module_reducer(module):
    ...
    return module_unpickle, (spec.name, spec.origin,
...
class DaliCallbackPickler(pickle.Pickler):
    ...
    def reducer_override(self, obj):
        ...
        if inspect.isfunction(obj):
            if isinstance(
                    obj,
                    type(dummy_lambda)) and obj.__name__ == dummy_lambda.__name__ or getattr(
                    obj,
                    '_dali_pickle_by_value',
                    False):
