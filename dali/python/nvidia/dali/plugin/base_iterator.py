...
class _DaliBaseIterator(object):
    ...
    def __init__(
        ...
        assert np.all(
            np.equal(
                [pipe.max_batch_size for pipe in pipelines], self.batch_size
            )
        ...
        if self._enable_checkpointing:
            if any(p.is_restored_from_checkpoint for p in self._pipes):
                ...
                if not all(p.is_restored_from_checkpoint for p in self._pipes):
                    ...
                    data == all_iterator_data[0] for data in all_iterator_data
                ):
                    logging.warning(
            ...
            self._initial_checkpoints = [
                p._get_checkpoint(iterator_data=self._save_state())
                for p in self._pipes
            ]
    ...
    def _calculate_shard_sizes(self, shard_nums):
        ...
        shards_end = np.floor(
            (shard_nums + 1) * self._size_no_pad / self._shards_num
        )
    ...
    def _extract_from_reader_and_validate(self):
        if self._reader_name:
            readers_meta = [
                p.reader_meta(self._reader_name) for p in self._pipes
            ]
            ...
            def check_equality_and_get(input_meta, name, err_msg):
                assert np.all(
                    np.equal(
                        [meta[name] for meta in input_meta], input_meta[0][name]
                    )
                ), err_msg_gen(err_msg)
            ...
            if self._last_batch_policy == LastBatchPolicy.DROP:
                ...
                self._size = (
                    readers_meta[0]["epoch_size_padded"] // self._shards_num
                )
            else:
    ...
    def _remove_padded(self):
        ...
        if self._last_batch_policy == LastBatchPolicy.PARTIAL:
            ...
            left = self.batch_size - (
                self._counter
                - self._shard_sizes_per_gpu_initial[self._shards_id]
            )
    ...
    def _report_no_data_in_pipeline(self):
        ...
        # checkpoint.
        if all(
            not p.is_restored_from_checkpoint or p._first_iter
            for p in self._pipes
        ):
            raise RuntimeError(
    ...
    def checkpoints(self):
        ...
        if not self._enable_checkpointing:
            raise ValueError(
                "Cannot access checkpoints with checkpointing disabled"
            )
        if not self._ever_consumed:
            ...
            return [
                p._get_checkpoint(iterator_data=iterator_data)
                for p in self._pipes
            ]
    ...
    def reset(self):
        ...
        if self._counter >= self._size or self._size < 0:
            if (
                ...
                if self._reader_name:
                    ...
                    self._counter_per_gpu = (
                        self._counter_per_gpu + self._counter
                    )
            ...
            if self._reader_name:
                ...
                if (
                    ...
                    if self._size == 0:
                        ...
                        self._size = (
                            math.ceil(
                                max(self._shard_sizes_per_gpu) / self.batch_size
                            )
                            * self.batch_size
                        )
            ...
            for p in self._pipes:
                p.reset()
                if p.empty():
                    with p._check_api_type_scope(
                        types.PipelineAPIType.ITERATOR
                    ):
                        p.schedule_run()
        else:
            logging.warning(
