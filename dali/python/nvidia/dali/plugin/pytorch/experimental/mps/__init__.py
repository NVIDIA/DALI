...
class _DALIPipelineCallable:
    def __init__(self, runner):
        self._runner = runner

    def __call__(self, *args, **kwargs):
        self._runner.init_pipeline()
        bound_args = self._runner._signature.bind(self, *args, **kwargs)
        return self._runner._add_sample(bound_args.arguments)
...
class DALIPipelineRunner:
    ...
    def callable(self):
        """
        Get the callable object for the pipeline.
        """
        if not self._callable:
            self._callable = _DALIPipelineCallable(self)
