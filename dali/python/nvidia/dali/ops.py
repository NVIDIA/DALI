...
"""
    if schema.HasInputDox():
        for i in range(schema.MaxNumInput()):
            ...
            input_type_str = schema.GetInputType(
                i) + _supported_layouts_str(schema.GetSupportedLayouts(i))
            dox = schema.GetInputDox(i)
            input_name = schema.GetInputName(i)
            ret += _numpydoc_formatter(input_name,
                                       input_type_str, dox, optional) + "\n"
    else:
        for i in range(schema.MinNumInput()):
            input_type_str = "TensorList" + \
                _supported_layouts_str(schema.GetSupportedLayouts(i))
            dox = "Input to the operator."
            input_name = f"input{i}" if schema.MaxNumInput() > 1 else "input"
            ret += _numpydoc_formatter(input_name,
                                       input_type_str, dox, False) + "\n"
...
def _get_kwargs(schema):
    ...
    for arg in schema.GetArgumentNames():
        ...
        if schema.IsDeprecatedArg(arg):
            ...
            # Renamed and removed arguments won't show full documentation (only
            # warning box)
        ...
        # Try to get dtype only if not set already (renamed args go through a
        # different path, see above)
        if not dtype:
...
"""
        dox = "Input to the operator.\n"
        fmt = "TensorList" + \
            _supported_layouts_str(schema.GetSupportedLayouts(0))
...
def _docstring_generator_call(op_name):
    ...
    if schema.HasCallDox():
        ...
        ret = "See :meth:`nvidia.dali.ops." + op_full_name + \
    ...
    def is_call_arg(name, value):
        ...
        if isinstance(
            ...
            (str,
             list,
             tuple,
             nvidia.dali.types.ScalarConstant)):
            return False
        return not nvidia.dali.types._is_scalar_value(value)

    def to_scalar(scalar):
        return scalar.value if isinstance(
            scalar, nvidia.dali.types.ScalarConstant) else scalar
        ...
        if inputs is not None:
            ...
            for i in range(len(inputs)):
                ...
                if isinstance(inp, _ScalarConstant):
                    inputs[i] = _instantiate_constant_node(
                        default_input_device, inp)
            inputs = tuple(inputs)
    ...
    if not schema.IsTensorArgument(name):
        raise TypeError(
            "The argument `{}` for operator `{}` should not be a `DataNode` but a {}".format(
                name, op_name, _type_name_convert_to_string(
...
def python_op_factory(name, schema_name=None):
    class Operator(metaclass=_DaliOperatorMeta):
        def __init__(self, *, device="cpu", **kwargs):
            ...
            # Check for any deprecated arguments that should be replaced or
            # removed
            ...
            for arg_name in arg_names:
                ...
                if new_name:
                    if new_name in kwargs:
                        raise TypeError(
                            "Operator {} got an unexpected '{}' deprecated argument when '{}' was already provided".format(
        ...
        # Check if all list representing multiple input sets have the same
...
def _process_op_name(op_schema_name, make_hidden=False):
    # Two underscores (reasoning: we might want to have single underscores in
...
def _op_name(op_schema_name, api="fn"):
    ...
    if api == "fn":
        ...
        raise ValueError(
            '{} is not a valid DALI api name, try one of {"fn", "ops"}'.format(api))
...
def _load_ops():
    ...
    for op_reg_name in _cpu_gpu_ops:
        ...
        if not hasattr(module, op_name):
            ...
            # so it would be usable, but not documented as coming from other
            # module
            if make_hidden:
...
class _TFRecordReaderImpl():
    ...
    def __call__(self, *inputs, **kwargs):
        # We do not handle multiple input sets for Reader as they do not have
        # inputs
        if (len(inputs) > self._schema.MaxNumInput() or
                len(inputs) < self._schema.MinNumInput()):
            raise ValueError(
...
class PythonFunction(PythonFunctionBase):
    ...
    def current_stream():
        ...
        return _CUDAStream(
            nvidia.dali.python_function_plugin.current_dali_stream())
    ...
    def check_outputs(outputs, num_outputs):
        if num_outputs > 1:
            if not isinstance(outputs, tuple):
                raise TypeError(
                    "The output from a multi-output Python function operator must be a tuple, got: ",
                    type(outputs))
            if len(outputs) != num_outputs:
                raise ValueError(
                    f"Unexpected number of outputs from Python function operator - got {len(outputs)}, expected {num_outputs}")
    ...
    def function_wrapper_batch(
            ...
            to_dlpack,
            *dlpack_inputs):
        arrays = [[from_dlpack(dlpack) for dlpack in dl_input]
                  for dl_input in dlpack_inputs]
...
class DLTensorPythonFunction(PythonFunctionBase):
    ...
    def __init__(
            ...
            **kwargs):
        func = lambda *ts: DLTensorPythonFunction._function_wrapper_dlpack(
...
def _preprocess_inputs(inputs, op_name, device, schema=None):
    ...
    def is_input(x):
        ...
        return isinstance(
            x, (list)) and any(
            isinstance(
                y, _DataNode) for y in x) and all(
                isinstance(
                    y, (_DataNode, nvidia.dali.types.ScalarConstant)) for y in x)
        ...
        if not is_input(inp):
            ...
            if not isinstance(inp, nvidia.dali.types.ScalarConstant):
                try:
                    ...
                    raise TypeError(
                        """when calling operator {0}:
Input {1} is neither a DALI `DataNode` nor a list of data nodes but `{2}`.
Attempt to convert it to a constant node failed.""" .format(
                            op_name, idx, type(inp).__name__)) from ex

            if not isinstance(inp, _DataNode):
                inp = nvidia.dali.ops._instantiate_constant_node(
                    input_device, inp)
...
def _group_inputs(inputs):
    ...
    for input in inputs:
        ...
        if isinstance(input, _DataNode):
            ...
            raise TypeError(
                ("Argument to arithmetic operation not supported. Got {}, expected "
                 "a return value from other DALI Operator  or a constant value of type 'bool', 'int', "
                 "'float' or 'nvidia.dali.types.Constant'.").format(
                    str(
                        type(input))))
    if len(integers) == 0:
        integers = None
...
# Select the `gpu` device if at least one of the inputs is `gpu`,
# otherwise `cpu`.
