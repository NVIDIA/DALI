...
class _PadBase(Operator):
    ...
    def __init__(
        ...
        self.pad_left, self.pad_top, self.pad_right, self.pad_bottom = _PadBase.get_padding(padding)
        self.fill = fill
    ...
    def _kernel(self, data_input):
        ...
        Tensor
            Padded data.
        """
        input_height, input_width, _, data_input = data_input
...
class _ValidatePaddingMode(_ArgumentValidateRule):
    ...
    @classmethod
    def verify(cls, *, padding_mode, **_) -> None:
        if padding_mode not in PADDING_CLASS.keys():
            raise ValueError(
                f"Invalid padding mode, expected {list(PADDING_CLASS.keys())}"
                f", got {padding_mode}"
            )
