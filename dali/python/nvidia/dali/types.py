...
_known_types = {
    ...
    # DALIDataType.UINT64 : ("int", int), # everything else fits into the
    # Python int
    ...
    DALIDataType._DATA_TYPE_VEC: (
        "nvidia.dali.types.DALIDataType", _to_list(
            lambda x: DALIDataType(
                int(x))))
}
...
if _tfrecord_support:
    _known_types[DALIDataType.FEATURE] = (
        "nvidia.dali.tfrecord.Feature", tfrec.Feature)
...
# avoid importing NumPy if to_numpy_type is not called to break strong
# NumPy dependency
_numpy_types = None
...
dtype: DALIDataType, optional
    ...
    def __eq__(self, other):
        ...
        # Delegate the call to the `__eq__` of other object, most probably a
        # `DataNode`
    ...
    def __ne__(self, other):
        ...
        # Delegate the call to the `__ne__` of other object, most probably a
        # `DataNode`
    ...
    def __bool__(self):
        ...
        raise TypeError(
            ("DALI ScalarConstant must be converted to one of bool or int types: ({}) "
    ...
    def __float__(self):
        if self.dtype in _float_types:
            return self.value
        raise TypeError(
            ("DALI ScalarConstant must be converted to one of the float types: ({}) "
...
def _is_compatible_array_type(value):
    return _is_numpy_array(value) or _is_mxnet_array(
        value) or _is_torch_tensor(value)


def _preprocess_constant_array_type(value):
    if _is_mxnet_array(value):
        # mxnet ndarray is not directly compatible with numpy.ndarray, but
        # provides conversion
        value = value.asnumpy()
...
def ConstantNode(device, value, dtype, shape, layout, **kwargs):
    ...
    if _is_compatible_array_type(value):
        ...
        # At this point value is a numpy array or a torch tensor. They have
        # very similar API
...
def _is_scalar_value(value):
    ...
    if isinstance(value, (bool, int, float)):
        return True
    return not _is_compatible_array_type(
        value) or _is_scalar_shape(value.shape)
