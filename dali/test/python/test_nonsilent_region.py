...
def audio_data(audio_files):
    raw, _ = fn.readers.file(files=audio_files, seed=1234)
...
def check_nonsilence_operator(batch_size, cutoff_value, window_size, reference_power,
    ...
    test_utils.compare_pipelines(
        nonsilent_region_pipe(cutoff_value, window_size, reference_power, reset_interval,
                              batch_size=batch_size, num_threads=1, device_id=0, seed=42),
        NonsilenceRosaPipeline(batch_size, -cutoff_value, window_size, reference_power,
                               reset_interval),
        batch_size=batch_size, N_iterations=3, eps=eps)

check_nonsilence_operator(3, -10, 1024, 0.0003, 8192, 1024)
