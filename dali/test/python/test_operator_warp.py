    ...
    def warp_fn(img, matrix):
        ...
        out = cv2.warpAffine(img,
                             ...
                             flags=(cv2.INTER_LINEAR
                                    | cv2.WARP_INVERSE_MAP) if inv_map else cv2.INTER_LINEAR)
...
io_types = [(dali.types.UINT8, dali.types.UINT8), (dali.types.UINT8, dali.types.FLOAT),
            (dali.types.FLOAT, dali.types.UINT8), (dali.types.FLOAT, dali.types.FLOAT)]
...
def test_gpu_vs_cpu():
    ...
    for batch_size in [1, 4, 19]:
        for use_input in [False, True]:
            for (itype, otype) in io_types:
                inv_map = random.choice([False, True])
                print("Testing gpu vs cpu", "\nbatch size: ", batch_size, " matrix as input: ",
                      use_input, " input_type: ", itype, " output_type: ", otype, " map_inverse:",
                      inv_map)
                cpu_pipeline = WarpPipeline("cpu",
                                            batch_size,
                                            otype,
