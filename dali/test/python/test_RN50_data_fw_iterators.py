...
import nvidia.dali.plugin as nvidia_plugin
...
for iterator_name in Iterators:
    ...
    pipes = [RN50Pipeline(batch_size=BATCH_SIZE, num_threads=WORKERS, device_id=n,
                          num_gpus=N, data_paths=data_paths, prefetch=PREFETCH, fp16=FP16, nhwc=NHWC) for n in range(N)]
            ...
            for j in range(iters):
                ...
                if j % LOG_INTERVAL == 0:
                    print("{} {}/ {}, avg time: {} [s], worst time: {} [s], speed: {} [img/s]"
                        .format(iterator_name, j + 1, iters, data_time.avg, data_time.max_val, N * BATCH_SIZE / data_time.avg))
                end = time.time()
        else:
