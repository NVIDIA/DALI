...
from test_utils import generator_random_axes_for_3d_input
from test_utils import generator_random_imagelike_data
...
def check_pad_dynamic_axes(device, batch_size, num_threads, use_negative, use_empty):
    ...
    image_gen = generator_random_imagelike_data(
    ...
    for _ in range(3):
        ...
        for sample_idx in range(batch_size):
            ...
            shape = np.array(outs[2][sample_idx])
            pad1 = np.array(
                outs[3][sample_idx].as_cpu() if device == 'gpu' else
                outs[3][sample_idx])
            pad2 = np.array(
                outs[4][sample_idx].as_cpu() if device == 'gpu' else
                outs[4][sample_idx])
            fill_value = np.array(outs[5][sample_idx])
            in_sh = in_img.shape
