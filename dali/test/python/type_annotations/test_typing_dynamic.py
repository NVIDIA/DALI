...
def test_copy_tensor_constant():
    ...
    const_torch = ndd.copy(torch.full((1, 1), 6))
    const_np = ndd.copy(np.full((1, 1), 7))
    expect_tensor(const_int, const_float, const_list, const_tuple, const_torch, const_np)

    assert np.array_equal(np.array(const_int), np.full((10,), 1))
    assert np.array_equal(np.array(const_float), np.full((10,), 2.0))
    assert np.array_equal(np.array(const_list), np.full((10, 2), [2, 3]))
    assert np.array_equal(np.array(const_tuple), np.full((10, 2), [4, 5]))
    assert np.array_equal(np.array(const_torch), np.full((10, 1, 1), 6))
    assert np.array_equal(np.array(const_np), np.full((10, 1, 1), 7))
