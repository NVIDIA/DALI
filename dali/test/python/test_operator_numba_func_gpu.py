...
def numba_func_cuda_pipe(shapes, dtype, run_fn=None, out_types=None, in_types=None, outs_ndim=None, ins_ndim=None, blocks=None, threads_per_block=None, setup_fn=None, batch_processing=None):
    ...
    return numba_function(data, run_fn=run_fn, out_types=out_types, in_types=in_types, outs_ndim=outs_ndim, ins_ndim=ins_ndim, blocks=blocks, threads_per_block=threads_per_block, setup_fn=setup_fn, batch_processing=batch_processing, device='gpu')
...
def test_numba_func():
    ...
    args = [
        ...
        ([(10, 5), (10, 5)], np.uint8, set_all_values_to_255_sample, [dali_types.UINT8], [dali_types.UINT8], [2], [2], [1, 1, 1], [10, 5, 1], None, True, [np.full((10, 5), 255, dtype=np.uint8), np.full((10, 5), 255, dtype=np.uint8)]),
        ...
        ([(20, 5), (20, 5)], np.float32, set_output_to_input_plus_5_sample, [dali_types.FLOAT], [dali_types.FLOAT], [2], [2], [1, 1, 1], [20, 5, 1], None, True, [np.full((20, 5), 6, dtype=np.float32), np.full((20, 5), 6, dtype=np.float32)]),
        ...
        ([(20, 10), (20, 10)], np.float32, set_consecutive_values_sample, [dali_types.FLOAT], [dali_types.FLOAT], [2], [2], [1, 1, 1], [20, 10, 1], None, True, [np.arange(20*10, dtype=np.float32).reshape((20, 10)), np.arange(20*10, dtype=np.float32).reshape((20, 10))]),
    ]
