...
class MockTensor:
    def __init__(self, data, layout=None):
        ...
        self.dtype = D.dtype(data.dtype)
        self.layout = layout
        self.device = D.Device("cpu")
        self.data = data
...
def test_mock_operator_tensor():
    ...
    out = op.run(D.EvalContext().get(), addend=a)
    assert out == (MockTensor(np.int32([10, 20, 30])),)

    out = op.run(D.EvalContext().get(), b1, addend=a)
    assert out == (MockTensor(np.int32([11, 22, 33])),)

    out = op.run(D.EvalContext().get(), b1, b2, addend=a)
    assert out == (
        MockTensor(np.int32([11, 22, 33])),
        MockTensor(np.int32([14, 25, 36])),
    )

    out = op.run(D.EvalContext().get(), b1, b2, b3, addend=a)
    assert out == (
        MockTensor(np.int32([11, 22, 33])),
        MockTensor(np.int32([14, 25, 36])),
...
def test_mock_operator_batch():
    ...
    out = op.run(D.EvalContext().get(), addend=a, batch_size=1)
    assert out == (MockBatch([MockTensor(np.int32([10, 20, 30]))]),)

    out = op.run(D.EvalContext().get(), b1, addend=a, batch_size=1)
    assert out == (MockBatch([MockTensor(np.int32([11, 22, 33]))]),)

    out = op.run(D.EvalContext().get(), b1, b2, addend=a, batch_size=1)
    assert out == (
        MockBatch([MockTensor(np.int32([11, 22, 33]))]),
        MockBatch([MockTensor(np.int32([14, 25, 36]))]),
    )

    out = op.run(D.EvalContext().get(), b1, b2, b3, addend=a, batch_size=1)
    assert out == (
        MockBatch([MockTensor(np.int32([11, 22, 33]))]),
        MockBatch([MockTensor(np.int32([14, 25, 36]))]),
...
def test_invocation_tensor():
    ...
    inv.run(D.EvalContext().get())
    assert inv.values(D.EvalContext().get()) == (
        MockTensor(np.int32([11, 22, 33])),
        MockTensor(np.int32([14, 25, 36])),
        MockTensor(np.int32([17, 28, 39])),
    )
...
def test_invocation_lazy_result():
    ...
    assert inv[0].value(D.EvalContext().get()) == MockBatch(
        [MockTensor(np.int32([11, 22, 33])), MockTensor(np.int32([133, 244]))]
    )
    assert inv[1].value(D.EvalContext().get()) == MockBatch(
        [MockTensor(np.int32([14, 25, 36])), MockTensor(np.int32([155, 266]))]
    )
    assert inv[2].value(D.EvalContext().get()) == MockBatch(
