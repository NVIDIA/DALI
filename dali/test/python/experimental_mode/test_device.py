...
def test_default_device():
    ...
    assert ndd.Device.current() == ndd.Device("gpu")
...
def test_device():
    if _backend.GetCUDADeviceCount() == 0:
        raise SkipTest("At least 1 device needed for the test")
    with ndd.Device("gpu"):
        assert ndd.Device.current().device_type == "gpu"
        assert ndd.Device.current().device_id == 0
    assert ndd.Device.current().device_type == "gpu"
    assert ndd.Device.current().device_id == 0
...
def test_device_with_id():
    ...
    with ndd.Device("gpu", other_device_id):
        assert ndd.Device.current().device_type == "gpu"
        assert ndd.Device.current().device_id == other_device_id
...
def test_device_parse():
    ...
    with assert_raises(ValueError, glob="Invalid device id"):
        ndd.Device("cpu:99")  # invalid id
    with assert_raises(ValueError, glob="Invalid device id"):
        ndd.Device("gpu:10000000000")  # CUDA device id is 32-bit, so 10B is surely invalid
...
def test_device_parse_multi_gpu():
    ...
    assert ndd.Device("gpu:1") == ndd.Device("gpu", 1)
...
def test_device_same_as_in_torch_multi_gpu():
    ...
    assert ndd.Device("gpu").device_id == 1

    torch.cuda.set_device(0)
    assert ndd.Device("gpu").device_id == 0
