...
def _test_graph_single_device(device = 'cpu', device_id = 0):
    ...
    with tf.device('/{0}:{1}'.format(device, device_id)):
        ...
        labels = tf.reshape(tf.one_hot(labels, labels_size), [batch_size, labels_size])
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for _ in range(iterations):
            final_accuracy = final_accuracy + accuracy.eval(feed_dict={keep_prob: 1.0})
...
def _test_estimators_single_device(device = 'cpu', device_id = 0):
    skip_for_incompatible_tf()
    ...
    with tf.device('/{0}:{1}'.format(device, device_id)):
        ...
        classifier.train(input_fn = train_fn, steps = epochs * iterations)

        assert classifier.evaluate(
            input_fn=train_fn,
            steps = iterations)["accuracy"] > target
