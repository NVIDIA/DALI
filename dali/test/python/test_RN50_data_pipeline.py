...
class CommonPipeline(Pipeline):
    def __init__(self, data_paths, num_shards, batch_size, num_threads, device_id,
                 prefetch, fp16, random_shuffle, nhwc, dont_use_mmap, decoder_type,
                 decoder_cache_params, reader_queue_depth, shard_id):
        ...
        if decoder_type == 'roi':
            ...
            print(f'Using nvJPEG with cache (size : {cache_size} '
...
class WebdatasetPipeline(CommonPipeline):
    def __init__(self, data_paths, decoder_cache_params, shard_id, num_shards,
                 random_shuffle, dont_use_mmap, **kwargs):
...
parser.add_argument(
    '--remove_default_pipeline_paths', action='store_true',
    help="For all data pipeline types, remove the default values")
parser.add_argument(
    '--file_read_pipeline_paths', default=None, type=str, metavar='N',
    help='Add custom FileReadPipeline paths. Separate multiple paths by commas')
parser.add_argument(
    '--mxnet_reader_pipeline_paths', default=None, type=str, metavar='N',
...
parser.add_argument(
    '--caffe_read_pipeline_paths', default=None, type=str, metavar='N',
    help='Add custom CaffeReadPipeline paths. Separate multiple paths by commas')
parser.add_argument(
    '--caffe2_read_pipeline_paths', default=None, type=str, metavar='N',
    help='Add custom Caffe2ReadPipeline paths. Separate multiple paths by commas')
parser.add_argument(
    '--tfrecord_pipeline_paths', default=None, type=str, metavar='N',
...
parser.add_argument(
    '--webdataset_pipeline_paths', default=None, type=str, metavar='N',
...
parser.add_argument(
    '--system_id', default="localhost", type=str, metavar='N',
...
print("GPUs: {N}, batch: {BATCH_SIZE}, workers: {WORKERS}, prefetch depth: {PREFETCH}, "
...
for pipe_name in test_data.keys():
    ...
    for i, data_set in enumerate(test_data[pipe_name]):
        if GPU_ID is None:
            pipes = [
                pipe_name(batch_size=BATCH_SIZE, num_threads=WORKERS, device_id=n,
                          num_shards=NUMBER_OF_SHARDS, data_paths=data_set,
                          prefetch=PREFETCH, fp16=FP16, random_shuffle=READ_SHUFFLE,
                          dont_use_mmap=DISABLE_MMAP, nhwc=NHWC, decoder_type=DECODER_TYPE,
                          decoder_cache_params=DECODER_CACHE_PARAMS,
                          reader_queue_depth=READER_QUEUE_DEPTH,
                          shard_id=DALI_SHARD + n) for n in range(N)]
        else:
            pipes = [
                pipe_name(batch_size=BATCH_SIZE, num_threads=WORKERS, device_id=GPU_ID,
                          num_shards=NUMBER_OF_SHARDS, data_paths=data_set,
                          prefetch=PREFETCH, fp16=FP16, random_shuffle=READ_SHUFFLE,
                          dont_use_mmap=DISABLE_MMAP, nhwc=NHWC, decoder_type=DECODER_TYPE,
                          decoder_cache_params=DECODER_CACHE_PARAMS,
        ...
        for i in range(args.epochs):
            ...
            for j in range(iters):
                ...
                if j % LOG_INTERVAL == 0:
                    print(f"System {SYS_ID}, GPU {GPU_ID}, run {i}: "
