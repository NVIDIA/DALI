...
from nvidia.dali.plugin.jax.clu import (
    DALIGenericPeekableIterator as DALIPeekableIterator,
)
...
def test_jax_peekable_iterator_peek():
    ...
    # then
    assert iterator.element_spec == {
        "data": ArraySpec(dtype=jnp.int32, shape=batch_shape)
    }
...
def test_jax_peekable_iterator_peek_async_result_after_next():
    ...
    for i in range(5):
        ...
        time.sleep(
            0.1
        )  # wait before calling next to give time for peek to start
        output = iterator.next()
        peeked_output = peeked_output.result()
...
@raises(
    ValueError,
    glob="The shape or type of the output changed between iterations.",
)
...
def test_iterators_init_method_api_compatibility():
    ...
    peekalbe_iterator_init_args = inspect.getfullargspec(
        DALIPeekableIterator.__init__
    ).args
