...
def test_resize_upsample_scales_nearest(device):
    ...
    resize_fn = lambda input_data: \
        fn.experimental.tensor_resize(input_data, scales=scales, interp_type=types.INTERP_NN)
    run_and_compare(expected, data, device, resize_fn)


@params('cpu', 'gpu')
def test_resize_downsample_scales_nearest(device):
    # TODO(janton): What is the equivalent nearest_mode for the DALI implementation?
    if skip_failing:
...
def test_resize_upsample_sizes_nearest(device):
    ...
    # from onnx.backend.test.case.node.resize import interpolate_nd, nearest_coeffs
    # expected = interpolate_nd(data, lambda x, _: nearest_coeffs(x), output_size=sizes.astype(np.int64))
    expected = np.array(
        [[[[1., 1., 1., 1., 2., 2., 2., 2.],
           [1., 1., 1., 1., 2., 2., 2., 2.],
           [1., 1., 1., 1., 2., 2., 2., 2.],
    ...
    resize_fn = lambda input_data: \
        fn.experimental.tensor_resize(input_data, sizes=sizes, interp_type=types.INTERP_NN)
    run_and_compare(expected, data, device, resize_fn)
...
def test_resize_upsample_scales_linear(device):
    ...
    resize_fn = lambda input_data: \
        fn.experimental.tensor_resize(input_data, scales=scales,
...
def test_resize_upsample_sizes_cubic(device):
    ...
    # from onnx.backend.test.case.node.resize import interpolate_nd, cubic_coeffs
    # expected = interpolate_nd(data, lambda x, _: cubic_coeffs(x, A=-0.5), output_size=sizes.astype(np.int64))
    expected = np.array(
    ...
    resize_fn = lambda input_data: \
        fn.experimental.tensor_resize(input_data, sizes=sizes,
                                      interp_type=types.INTERP_CUBIC, antialias=False)
    run_and_compare(expected, data, device, resize_fn)
