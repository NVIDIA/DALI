...
def remap_pipe(remap_op, maps, img_size):
    ...
    mapx, mapy = fn.external_source(source=maps, batch=True, cycle=True, num_outputs=2)
    if remap_op == 'dali':
        return fn.remap(img.gpu(), mapx.gpu(), mapy.gpu(), interp=DALIInterpType.INTERP_NN,
                        device='gpu')
    elif remap_op == 'cv':
        return fn.python_function(img, mapx, mapy, function=cv_remap)
    else:
...
class RemapTest(unittest.TestCase):
    ...
    @params('identity', 'xyflip')
    # @params('identity', 'reduce', 'xflip', 'yflip', 'xyflip')  # TODO random
    def test_reduce(self, map_mode):
        maps = [update_map(mode=map_mode, shape=self.img_size, nimages=self.batch_size)]
        dpipe = remap_pipe('dali', maps, self.img_size, batch_size=self.batch_size, num_threads=3,
                           device_id=0, exec_async=False, exec_pipelined=False)
        cpipe = remap_pipe('cv', maps, self.img_size, batch_size=self.batch_size, num_threads=3,
                           device_id=0, exec_async=False, exec_pipelined=False)
        self.compare_pipelines_pixelwise(dpipe, cpipe, N_iterations=3, eps=.01)

    def compare_pipelines_pixelwise(self, pipe1, pipe2, N_iterations, eps=.01):
        pipe1.build()
        pipe2.build()
