...
def _cred(dali_var, aws_var, mock_default):
    ...
    DALI_TEST_S3_* wins, then whatever the environment already carries for the AWS SDK, so
    pointing DALI_TEST_S3_ENDPOINT at MinIO or at real S3 works with the usual AWS variables
    and does not need test-specific ones.
    """
    return os.environ.get(dali_var) or os.environ.get(aws_var) or mock_default
...
REGION = _cred("DALI_TEST_S3_REGION", "AWS_DEFAULT_REGION", "us-east-1")
...
def delete_prefix(client, bucket, prefix):
    """Deletes every object under `prefix`. Paginates: a page holds at most 1000 keys."""
...
def skip_if_no_s3_support():
    ...
    def probe():
        return tuple(fn.readers.file(file_root="s3://dali-no-such-bucket/no-such-prefix"))
