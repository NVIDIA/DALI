...
def coco_rotate_pipeline(angle=45.0, keep_size=False, ltrb=True):
    image, boxes, labels = fn.readers.coco(
        file_root=str(file_root), annotations_file=str(train_annotations), ltrb=ltrb, ratio=True
    )
    ...
    boxes_rot, labels_rot = fn.bbox_rotate(
        boxes, labels, angle=angle, ltrb=ltrb, keep_size=keep_size
    )
    return image, boxes, labels, image_rot, boxes_rot, labels_rot


def test_bbox_rotate():
    pipeline = coco_rotate_pipeline()
    pipeline.build()
    outs = pipeline.run()
