...
def input_batch(num_dim):
    ...
    for batch_size in batch_sizes:
        yield [
            rng.random(rng.integers(low=0, high=50, size=num_dim))
...
def run_pipeline(device, num_dim, replace=False, layout=None):
    ...
    pipe.build()
    expected_layout = (
        "F" + "*" * (num_dim - 1) if layout is None else "F" + layout[1:]
    )
    for baseline in input_batch(num_dim):
        (out,) = pipe.run()
        check_batch(
            out, baseline, len(baseline), expected_layout=expected_layout
        )
...
@raises(
    RuntimeError,
    "Per-frame argument input must be a sequence. "
    ...
    def pipeline():
        rng = fn.external_source(
            lambda info: np.array(
                [info.iteration, info.iteration + 1], dtype=np.float32
            ),
            batch=False,
        )
        return fn.per_frame(
            fn.random.uniform(range=rng, device="gpu", shape=(1, 1, 1), seed=42)
        )
...
def test_pass_through():
    for _ in range(
        50
    ):  # repeat the test as if it is wrong it does not manifest deterministically
        _test_pass_through()
