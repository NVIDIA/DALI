...
def random_shape(max_shape, diff=100):
    ...
    for s in max_shape:
        assert s > diff
    return np.array(
        [np.random.randint(s - diff, s) for s in max_shape], dtype=np.int32
    )
...
def check_coin_flip(
    ...
    with pipe:
        ...
        outputs = [
            dali.fn.random.coin_flip(
                *inputs, device=device, probability=p, shape=shape_arg
            )
        ]
    ...
    if max_shape is not None:
        shapes_out = (
            outputs[1].as_cpu()
            if isinstance(outputs[1], TensorListGPU)
            else outputs[1]
        )
