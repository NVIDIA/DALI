...
from nvidia.dali._multiproc.pool import WorkerPool, WorkBatch
...
class MockGroup:
    ...
    def __init__(self, source_desc, batch, prefetch_queue_depth, bytes_per_minibatch_hint):
    ...
    @classmethod
    def from_callback(cls, callback, batch=False, prefetch_queue_depth=1, bytes_per_minibatch_hint=None):
        _, source_desc = get_callback_from_source(callback, cycle=None)
        return cls(source_desc, batch, prefetch_queue_depth, bytes_per_minibatch_hint)
...
def test_pool_one_task(start_method):
    ...
    with create_pool(groups, keep_alive_queue_size=1, num_workers=1, start_method=start_method) as pool:
        pids = get_pids(pool)
        pid = pids[0]
        tasks = [(SampleInfo(0, 0, 0, 0),)]
        pool.schedule_batch(context_i=0, dst_chunk_i=0, work_batch=WorkBatch.sample_mode(tasks))
...
def test_pool_work_split_multiple_tasks(start_method):
    ...
    with create_pool(callbacks, keep_alive_queue_size=1, num_workers=2, start_method=start_method) as pool:
        num_tasks = 16
        pids = get_pids(pool)
        assert len(pids) == 2
...
def test_pool_iterator_dedicated_worker(start_method):
    ...
    with create_pool(groups, keep_alive_queue_size=1, num_workers=4, start_method=start_method) as pool:
        pids = get_pids(pool)
        assert len(pids) == 4
        tasks_list = []
        samples_count = 0
        for i in range(4):
            ...
            pool.schedule_batch(context_i=0, dst_chunk_i=i, work_batch=WorkBatch.sample_mode(tasks))
            pool.schedule_batch(context_i=1, dst_chunk_i=i, work_batch=WorkBatch.batch_mode((i,)))
        assert pool.contexts[0].setup.dedicated_worker is None
        iter_worker_num = pool.contexts[1].setup.dedicated_worker
...
def test_pool_many_ctxs(start_method):
    ...
    with create_pool(groups, keep_alive_queue_size=1, num_workers=1, start_method=start_method) as pool:
        ...
        tasks = [(SampleInfo(0, 0, 0, 0),)]
        pool.schedule_batch(context_i=0, dst_chunk_i=0, work_batch=WorkBatch.sample_mode(tasks))
        pool.schedule_batch(context_i=1, dst_chunk_i=0, work_batch=WorkBatch.sample_mode(tasks))
        batch_0 = pool.receive_batch(context_i=0)
        batch_1 = pool.receive_batch(context_i=1)
...
def test_pool_context_sync(start_method):
    ...
    with create_pool(groups, keep_alive_queue_size=1, num_workers=4, start_method=start_method) as pool:
        ...
        for i in range(4):
            ...
            pool.schedule_batch(context_i=0, dst_chunk_i=i, work_batch=WorkBatch.sample_mode(tasks))
            pool.schedule_batch(context_i=1, dst_chunk_i=i, work_batch=WorkBatch.sample_mode(tasks))
        assert len(pool.contexts[0].scheduled) == 4
        assert len(pool.contexts[1].scheduled) == 4
        ...
        pool.reset()
        tasks = [(SampleInfo(1000 + j, 0, 0, 0),) for j in range(5)]
        pool.schedule_batch(context_i=0, dst_chunk_i=0, work_batch=WorkBatch.sample_mode(tasks))
        pool.schedule_batch(context_i=1, dst_chunk_i=0, work_batch=WorkBatch.sample_mode(tasks))
        assert len(pool.contexts[0].scheduled) == 1
        assert len(pool.contexts[1].scheduled) == 1
        batch_0 = pool.receive_batch(context_i=0)
        batch_1 = pool.receive_batch(context_i=1)
        assert len(batch_0) == len(tasks)
        assert len(batch_1) == len(tasks)
...
def _test_multiple_stateful_sources_single_worker(num_workers):
    ...
    with create_pool(groups, keep_alive_queue_size=1, num_workers=num_workers, start_method="spawn") as pool:
        ...
        pool.schedule_batch(context_i=0, dst_chunk_i=0, work_batch=WorkBatch.batch_mode((0,)))
        pool.schedule_batch(context_i=1, dst_chunk_i=0, work_batch=WorkBatch.batch_mode((0,)))
        iter_worker_num_0 = pool.contexts[0].setup.dedicated_worker
        iter_worker_num_1 = pool.contexts[1].setup.dedicated_worker
