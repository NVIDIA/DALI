...
def build_src_pipe(device, layout=None):
    ...
    out_batches = fn.external_source(
        source=batches, device=device, cycle=True, layout=layout
    )
...
def _test_feed_input(device, is_serialized):
    ...
    dst_pipe = Pipeline(
        batch_size, 1, 0, exec_async=False, exec_pipelined=False
    )
...
def _test_callback(device, as_tensors, change_layout_to=None):
    ...
    outs = fn.external_source(
        source=get_from_src, device=device, layout=change_layout_to
    )
...
class BatchCb:
    ...
    def __call__(self, arg):
        if self.batch_info:
            ...
            assert isinstance(
                arg, int
            ), "Expected integer as cb argument, got {}".format(arg)
            iteration = arg
            epoch_idx = -1
        if iteration >= self.epoch_size:
            raise StopIteration
        return [
            np.array([iteration, epoch_idx], dtype=np.int32)
...
def _test_epoch_idx(batch_size, epoch_size, cb, batch_info, batch_mode):
    ...
    with pipe:
        ext = fn.external_source(
            source=cb, batch_info=batch_info, batch=batch_mode
        )
        pipe.set_outputs(ext)
    pipe.build()
...
@raises(
    RuntimeError,
    glob="ExternalSource expected data of type uint8 and got: float",
)
...
def test_ndim_arg():
    ...
    src_data = [[np.ones((120, 120, 3), dtype=np.uint8)] * batch_size]
    src_pipe = Pipeline(batch_size, 1, 0)
    src_ext1 = fn.external_source(
        source=src_data, dtype=DALIDataType.UINT8, ndim=3
    )
    src_ext2 = fn.external_source(
        source=src_data, dtype=DALIDataType.UINT8, layout="HWC"
    )
...
@raises(
    RuntimeError,
    glob="ExternalSource expected data with 3 dimensions and got 2 dimensions",
)
...
def _test_partially_utilized_external_source_warning(usage_mask, source_type):
    ...
    def pipeline():
        ...
        utilized_outputs = (
            out for out, is_used in zip(outputs, usage_mask) if is_used
        )
        return tuple(
    ...
    unused_output_idxs = [
        i for i, is_used in enumerate(usage_mask) if not is_used
    ]
...
def test_partially_utilized_external_source_warning():
    ...
    for num_outputs in (2, 3, 4):
        for num_unused in range(1, num_outputs):
            ...
            yield _test_partially_utilized_external_source_warning, usage_mask, next(
                source_type
            )
...
def _test_partially_utilized_es_old_style(usage_mask):
    ...
    num_outputs = len(usage_mask)
    batch_size = 16
    batch = np.array(list(range(batch_size * 1024))).reshape(
        batch_size, 16, 16, 4
    )
    ...
    class OldStylePipe(Pipeline):
        ...
        def define_graph(self):
            ...
            self.utilized_inputs = [
                inp
                for inp, is_used in zip(self.all_inputs, usage_mask)
                if is_used
            ]
...
def test_empty_es():
    ...
    with assert_raises(
        RuntimeError, glob="*ExternalSource expects non-empty batches*"
    ):
        pipe = pipeline(batch_size=max_batch_size, num_threads=4, device_id=0)
        pipe.build()
        pipe.run()
...
def to_tensor_list_gpu(data):
    @pipeline_def(
        batch_size=len(data), num_threads=4, device_id=0, prefetch_queue_depth=1
    )
    def convert_pipe():
...
def test_repeat_last():
    ...
    def pipeline():
        ...
        gpu = fn.external_source(
            name="es_gpu", repeat_last=True, device="gpu", no_copy=True
        )
        return cpu, gpu

    pipe = pipeline(
        batch_size=4, num_threads=4, device_id=0, prefetch_queue_depth=1
    )
    pipe.build()
    data1 = [
...
def test_repeat_last_queue():
    ...
    pipe = pipeline(
        batch_size=4, num_threads=4, device_id=0, prefetch_queue_depth=2
    )
    pipe.build()
    data1 = [
        np.array([1], dtype=np.int32),
...
def _check_blocking(device):
    ...
    for _ in range(5):
        ...
        data_to_feed = rng.random(size=(batch_size, 4, 6, 2)).astype(
            dtype=np.int32
        )
        pipe.feed_input("test_source", data_to_feed)
...
def test_decorated_external_source():
    ...
    def test_pipe():
        ...
        src_2 = fn.external_source(
            source=SourceClassWithoutInfo(42), batch=False
        )
...
@raises(
    TypeError, glob="Found var-positional argument `*args` which is not allowed"
)
...
@raises(
    TypeError, glob="Found var-keyword argument `**kwargs` which is not allowed"
)
...
@raises(
    TypeError,
    glob="Found keyword-only argument `kw_only` which is not allowed.",
)
...
@raises(
    TypeError,
    glob="Found more than one positional argument, which is not allowed.",
)
