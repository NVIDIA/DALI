...
auto img_gray    = dali_extra + "/db/single/bmp/0/cat-111793_640_grayscale.bmp";
auto img_palette = dali_extra + "/db/single/bmp/0/cat-111793_640_palette_8bit.bmp";
}  // namespace
...
TYPED_TEST(OpenCVFallbackDecodeTest, DecodeROI) {
  using OutputType = TypeParam;
  ...
  ASSERT_FALSE(roi);
  roi.begin = { 13, 17 };
  roi.end = { info.shape[0] - 23, info.shape[1] - 19 };
  ASSERT_TRUE(roi);
  TensorShape<> out_shape;
  out_shape = shape_cat(roi.shape(), 3);
  ...
  cv::Mat m = cv::imread(fname, cv::IMREAD_COLOR | cv::IMREAD_IGNORE_ORIENTATION);
  cv::Rect roi_rect(cv::Point2i(roi.begin[1], roi.begin[0]),
                    cv::Point2i(roi.end[1],   roi.end[0]));
  m = m(roi_rect);
  cv::cvtColor(m, m, cv::COLOR_BGR2RGB);
