...
void SliceKernelImplChannelLast(OutputType *output, const InputType *input,
                                ...
                                const int64_t *out_shape, const int64_t *in_shape,
                                const int64_t *anchor, const OutputType *fill_values,
...
template <typename OutputType, typename InputType, bool OutOfBounds, bool NeedPad>
void SliceKernelImpl(OutputType *output, const InputType *input, const int64_t *out_strides,
  ...
  if (OutOfBounds) {
    ...
    if (NeedPad) {
      ...
      for (; in_idx < 0 && out_idx < out_shape[d];
           in_idx += in_strides[d], out_idx += out_strides[d]) {
    ...
    // within input bounds
    for (; in_idx < in_shape[d] && out_idx < out_shape[d];
         in_idx += in_strides[d], out_idx += out_strides[d]) {
    ...
    if (NeedPad) {
      ...
      for (; out_idx < out_shape[d]; in_idx += in_strides[d], out_idx += out_strides[d]) {
        output[out_idx] = *fill_values;
        if (d == channel_dim)
          fill_values++;
      }
...
void SliceKernelImpl(OutputType *output, const InputType *input, const int64_t *out_strides,
  ...
  if (NeedPad) {
    ...
    for (; in_idx < 0 && out_idx < out_shape[d]; in_idx++, out_idx++) {
      SliceKernelImpl(
          output, input, out_strides + 1, in_strides + 1, out_shape + 1, in_shape + 1, anchor + 1,
...
void SliceKernel(OutputType *output, const InputType *input, const TensorShape<Dims> &out_strides,
                 const TensorShape<Dims> &in_strides, const TensorShape<Dims> &out_shape,
                 const TensorShape<Dims> &in_shape, const TensorShape<Dims> &anchor,
  ...
  if (need_pad) {
    slice_impl::SliceKernelImpl(
        ...
        anchor.data(), fill_values, channel_dim, std::integral_constant<int, Dims>(),
...
DLL_LOCAL  // workaround for GCC bug: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=80947
    void
    SliceKernel(ExecutionEngine &exec_engine, OutputType *out_data, const InputType *in_data,
                const TensorShape<Dims> &out_strides, const TensorShape<Dims> &in_strides,
                const TensorShape<Dims> &out_shape, const TensorShape<Dims> &in_shape,
                const SliceArgs<OutputType, Dims> &args, int min_blk_sz = kSliceMinBlockSize,
                int req_nblocks = -1) {
  if (req_nblocks < 0)
  ...
  if (CanRunPlainCopy<OutputType, InputType, Dims>(out_strides, in_strides, out_shape, in_shape,
    ...
    for (int b = 0; b < nblocks; b++) {
      ...
      exec_engine.AddWork(
          [=](int tid) { std::memcpy(out_data + b_start, in_data + b_start, b_nbytes); }, b_nbytes,
          false);  // do not start work immediately
    }
    return;
  }
  ...
  ForEachBlock(start, end, split_factor, 0, LastSplitDim(split_factor),
               [&](const TensorShape<Dims> &blk_start, const TensorShape<Dims> &blk_end) {
                 ...
                 exec_engine.AddWork(
                     [=](int) {
                       SliceKernel(output_ptr, in_data, out_strides, in_strides, blk_shape,
                                   in_shape, blk_anchor, GetPtr<OutputType>(args.fill_values),
                                   args.channel_dim);
                     },
 ...
 */
template <typename OutputType, typename InputType, int Dims>
void SliceKernel(SequentialExecutionEngine &exec_engine, OutputType *out_data,
                 const InputType *in_data, const TensorShape<Dims> &out_strides,
                 const TensorShape<Dims> &in_strides, const TensorShape<Dims> &out_shape,
                 const TensorShape<Dims> &in_shape, const SliceArgs<OutputType, Dims> &args,
...
class SliceCPU {
 public:
   ...
   * ``req_nblocks`` or the block volume is smaller than the minimum practical size, ``min_blk_sz``.
  ...
  void Schedule(KernelContext &context, OutTensorCPU<OutputType, Dims> out,
                InTensorCPU<InputType, Dims> in, const SliceArgs<OutputType, Dims> &args,
                ExecutionEngine &exec_engine, int min_blk_sz = kSliceMinBlockSize,
                int req_nblocks = -1) {
    ...
    if (fill_values_size > 1) {
      DALI_ENFORCE(
          args.channel_dim >= 0 && args.channel_dim < Dims,
  ...
  void Run(KernelContext &context, OutTensorCPU<OutputType, Dims> out,
           InTensorCPU<InputType, Dims> in, const SliceArgs<OutputType, Dims> &slice_args) {
