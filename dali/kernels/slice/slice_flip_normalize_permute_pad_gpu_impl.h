...
#ifndef DALI_KERNELS_SLICE_SLICE_FLIP_NORMALIZE_PERMUTE_PAD_GPU_IMPL_H_
#define DALI_KERNELS_SLICE_SLICE_FLIP_NORMALIZE_PERMUTE_PAD_GPU_IMPL_H_
...
KernelRequirements SliceFlipNormalizePermutePadGpu<OutputType, InputType, Dims>::Setup(
    KernelContext &context, const InListGPU<InputType, Dims> &in,
...
template <typename OutputType, typename InputType, int Dims>
void SliceFlipNormalizePermutePadGpu<OutputType, InputType, Dims>::Run(
    KernelContext &context, const OutListGPU<OutputType, Dims> &out,
  ...
  auto *block_descs_cpu = context.scratchpad->AllocatePinned<slice_impl::BlockDesc>(block_count_);
  ...
  for (int i = 0; i < in.size(); i++) {
    ...
    sample_desc.need_pad = NeedPad(Dims, processed_args.anchor.data(),
                                   processed_args.in_shape.data(), processed_args.out_shape.data());
  ...
  BOOL_SWITCH(need_pad, NeedPad, (
    BOOL_SWITCH(need_flip, NeedFlip, (
      BOOL_SWITCH(need_normalize_, NeedNormalize, (
        ...
        <<<grid, kBlockDim, 0, context.gpu.stream>>>(
            sample_descs_gpu, block_descs_gpu);));  // NOLINT
      )
    )  // NOLINT
  ));  // NOLINT
...
#endif  // DALI_KERNELS_SLICE_SLICE_FLIP_NORMALIZE_PERMUTE_PAD_GPU_IMPL_H_
