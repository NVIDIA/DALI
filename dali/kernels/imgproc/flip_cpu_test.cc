...
class FlipCpuTest
 ...
 protected:
  ...
  int flip_y_;
  int flip_z_;
  TensorShape<5> shape_;
  std::vector<float> data_;
  OutTensorCPU<float, 5> in_view_;
};
...
TEST_P(FlipCpuTest, KernelTest) {
  ...
  auto out_view = OutTensorCPU<float, 5>(out_data.data(), out_shape);
...
INSTANTIATE_TEST_SUITE_P(FlipCpuTest, FlipCpuTest, testing::Combine(
    ...
    testing::Values(0, 1),
    testing::Values(0, 1),
    testing::Values(std::array<Index, 5>{1, 8, 9, 9, 3}, std::array<Index, 5>{2, 3, 18, 18, 2})));
