...
class FlipGpuTest: public testing::TestWithParam<std::array<Index, 5>> {
 ...
 protected:
  ...
  TensorShape<5> tensor_shape_;
  size_t volume_;
  TensorListShape<5> shape_;
  TestTensorList<float, 5> ttl_in_;
  TestTensorList<float, 5> ttl_out_;
};
...
TEST_P(FlipGpuTest, ImplTest) {
  ...
  ttl_out_.reshape(reqs.output_shapes[0].to_static<5>());
  auto out_view = ttl_out_.gpu();
  for (int i = 0; i < in_view.num_samples(); ++i) {
    detail::gpu::FlipImpl(
        out_view.tensor_data(i), in_view.tensor_data(i),
        tensor_shape_[0], tensor_shape_[1], tensor_shape_[2], tensor_shape_[3], tensor_shape_[4],
...
INSTANTIATE_TEST_SUITE_P(FlipGpuTest, FlipGpuTest,
    ::testing::ValuesIn({
        std::array<Index, 5>{4, 1, 2, 2, 10},
        std::array<Index, 5>{4, 1, 2, 2, 2},
        std::array<Index, 5>{1, 4, 9, 18, 3},
        std::array<Index, 5>{1, 3, 18, 9, 4}}));
