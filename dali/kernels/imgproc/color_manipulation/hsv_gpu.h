...
#ifndef DALI_KERNELS_IMGPROC_COsdfsdfdsfSS_CONTRAST_GPU_H_
#define DALI_KERNELS_IMGPROC_COsdfsdfdsfSS_CONTRAST_GPU_H_
...
CreateSampleDescriptors(const OutListGPU<OutputType, ndims> &out,
                        const InListGPU<InputType, ndims> &in, const std::vector<float> &hue,
                        const std::vector<float> &saturation, const std::vector<float> &value) {
  ...
  for (int i = 0; i < in.num_samples(); i++) {
    ...
    sample.hue = hue[i];
    sample.saturation = saturation[i];
    sample.value = value[i];
  }
...
class HsvGpu {
 ...
 public:
  ...
  KernelRequirements Setup(KernelContext &context, const InListGPU<InputType, ndims> &in,
                           const std::vector<float> &hue, const std::vector<float> &saturation,
  ...
  void Run(KernelContext &context, const OutListGPU<OutputType, ndims> &out,
           ...
           const std::vector<float> &saturation, const std::vector<float> &value,
           const std::vector<Roi<spatial_dims>> &rois = {}) {
    auto sample_descs = CreateSampleDescriptors(out, in, hue, saturation, value);
