...
#include "dali/kernels/imgproc/color_manipulation/hsv_gpu.h"
...
class HsvGpuTest : public ::testing::Test {
...
INPUT_OUTPUT_TYPED_TEST_SUITE(HsvGpuTest, TestTypes);

namespace {

template <class GtestTypeParam>
using TheKernel = hsv::HsvGpu<typename GtestTypeParam::Out, typename GtestTypeParam::In>;

}  // namespace


TYPED_TEST(HsvGpuTest, check_kernel) {
  check_kernel<TheKernel<TypeParam>>();
}


//TYPED_TEST(HsvGpuTest, setup_test) {
...
//TYPED_TEST(HsvGpuTest, run_test) {
//  TheKernel<TypeParam> kernel;
//  KernelContext c;
//  InListGPU<typename TypeParam::In, kNdims> in(this->input_device_, this->shapes_);
...
//TYPED_TEST(HsvGpuTest, roi_to_TensorListShape) {
...
//TYPED_TEST(HsvGpuTest, adjust_empty_rois) {
...
//TYPED_TEST(HsvGpuTest, adjust_rois) {
//  constexpr size_t ndims = 3;
//
//  std::vector<Roi<ndims - 1>> rois = {
//          {{1, 2}, {3, 4}},
...
//TYPED_TEST(HsvGpuTest, sample_descriptors) {
//  {
//    InListGPU<typename TypeParam::In, kNdims> in(this->input_device_, this->shapes_);
//    OutListGPU<typename TypeParam::Out, kNdims> out(this->output_,
