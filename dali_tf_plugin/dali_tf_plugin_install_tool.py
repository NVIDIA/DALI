...
class InstallerHelper:
    def __init__(self, plugin_dest_dir=None):
        ...
        self.is_compatible_with_prebuilt_bin = (
            ...
            and self.platform_machine == "x86_64"
        )
        ...
        if self.can_install_prebuilt:
            self.prebuilt_plugins_available = find(
                "libdali_tf_*.so", self.prebuilt_dir
            )
            ...
            if best_version is None:
                ...
                self.prebuilt_plugin_best_match = os.path.join(
                    self.prebuilt_dir, self.plugin_name
                )
    ...
    def debug_str(self):
        ...
        s += "\n TF path:                              {}".format(
            self.tf_path or "Not Installed"
        )
        s += "\n DALI TF plugin destination directory: {}".format(
            self.plugin_dest_dir
        )
        s += "\n Is Conda environment?                 {}".format(
            "Yes" if self.is_conda else "No"
        )
        s += '\n Using compiler:                       "{}", version {}'.format(
            self.cpp_compiler, self.default_cpp_version or "Unknown"
        )
        s += "\n TF version installed:                 {}".format(
            self.tf_version or "Unknown"
        )
        if self.tf_version:
            s += "\n g++ version used to compile TF:       {}".format(
                self.tf_compiler or "Unknown"
            )
    ...
    def _test_plugin_in_tmp_dir(self, lib_path, dali_stub, test_fn):
        ...
        with tempfile.TemporaryDirectory(
            ...
            try:
                ...
                if ret != 0:
                    print(
                        f"Failed to import TF library, importing returned {ret}"
                    )
                return ret == 0
    ...
    def check_load_plugin(self, lib_path, dali_stub):
        ...
        return self._test_plugin_in_tmp_dir(
            lib_path, dali_stub, tf.load_op_library
        )

    def test_plugin(self, lib_path, dali_stub):
        return self._test_plugin_in_tmp_dir(
            lib_path, dali_stub, plugin_load_and_test
        )
    ...
    def install_prebuilt(self):
        ...
        print(
            f"Tensorflow was built with g++ {self.tf_compiler}, providing prebuilt plugin"
        )

        if self.check_plugin(
            self.prebuilt_plugin_best_match, self.prebuilt_dali_stub
        ):
            print(
                "Copy {} to {}".format(
                    self.prebuilt_plugin_best_match, self.plugin_dest_dir
                )
            )
    ...
    def get_compiler(self):
        ...
        if not self.can_default_compile:
            if self.has_alt_compiler:
                print(
                    "Will use alternative compiler {}".format(self.alt_compiler)
                )
    ...
    def build(self):
        ...
        with tempfile.TemporaryDirectory(prefix="dali_stub_") as tmpdir:
            ...
            dali_c_api_hdr = os.path.join(
                self.src_path, "include", "dali", "c_api.h"
            )
            ...
            subprocess.check_call(
                cmd, cwd=self.src_path, shell=False
            )  # nosec B603
            ...
            for filename in filenames:
                plugin_src = (
                    plugin_src + " " + os.path.join(self.src_path, filename)
                )
            ...
            cpp_ver = (
                "--std=c++14"
                if self.tf_version < LooseVersion("2.10")
                else "--std=c++17"
            )
            ...
            root_include = (
                "-I" + os.getenv("PREFIX", default="/usr") + "/include"
            )
            cmd = [
                compiler,
                "-Wl,-R,$ORIGIN/..",
